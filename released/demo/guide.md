...
###### <sub>VLOOK™</sub><br />让你的 Markdown 有了新`{看}(wán)`法<br />──<br /><sup>快速参考手册</sup><br />`#最新版本|V13.0#(theme2!)`<br /><br /><br />**MAX°孟兆**<br />*COPYRIGHT © 2016-2021. MAX°DESIGN.*
...
| **出版辅助工具** | 适用场景                                                     |
| :--------------: | ------------------------------------------------------------ |
|  长内容自动折叠  | 插图、表格、代码内容过高的情况下会自动进行折叠[ # ](#长内容自动折叠) |
...
| -------------------------------------------------- | ---- | ------------------------------------------------------------ |
| **模板主题、字体主题**                             |      | **不杂项**                                                   |
| 为不同类型的文档设置不同的模板主题[ # ](#模板主题) | :    | 支持 Typora 编辑时的特性增强[ # ](#Typora 编辑时特性增强)    |
| 为文档选择不同的字体主题[ # ](#字体主题)           | :    | 支持[跨平台、动效](#跨平台、动效支持)（如：毛玻璃） [emoji](#emoji 支持)、[国际化](#国际化支持) |
| Dark Mode is NOW[ # ](#Dark Mode)                  | :    | 支持快捷键操作[ # ](#快捷键操作汇总)                         |
| 为文档添加封面、封底[ # ](#封面、封底)             | :    | 预置插件参数，自定义你的文档表现[ # ](#预置插件参数)         |
| 更美观的公式[ # ](#更美观的公式)                   | :    |                                                              |
...
[<kbd>![](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-back.svg?mode=icon&fill=text) 我要回到 快速入坑°文档排版</kbd>](#快速入坑°文档排版)
...
| :----------------: | ----------------- | ----------------- | ---------------------- | ---------------------------- |
| 单元格（横向合并） | ==                | 单元格 afgiklo 10 | 单元格                 | 单元格 `#徽章名称|徽章内容#` |
...
|     A      | 单元格 afgiklo 10       | 单元格 afgiklo 10               | 单元格                 | 单元格 `#徽章名称|徽章内容#` |
...
> 只需要在表头首列的单元格内容的开关输入表头重复标识 `## (空格)`，则该表格的表头就会自动根据预置的规则进行重复显示。
>
> 
>
> **预置规则：**
...
「**用 Markdown 写文档，想通过「标签」或「徽章」突出或组织相关性较强的内容，这个想法会不会不现实呢？**」

`#VLOOK|say#(theme1!)` 对 Markdown 行内代码（Code）语法 `` ` ` `` 进行了扩展，满足实用的「标签」和「徽章」排的版需求！

例如：可以这样 `#标签#(orange)`，还可以这样`#这是徽章名称|这是徽章内容#(pink)`，继续往下看看就知道这是怎么做到的～
...
> 其中：
>
> `color`：指定标签显示对应的预置颜色标识，预置颜色标识与「[彩虹标签](#彩虹标签)」的颜色标识保持一致。同时，颜色标识支持添加 `!` 启用强调样式
...
>
> 这是一个 `#彩虹引用|红色#(red)`，颜色标识为：`red`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(red)`

> ###### ![red](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-red.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|红色强调#(red!)`，颜色标识为：`red!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(red!)`
...
>
> 这是一个 `#彩虹引用|橙色#(orange)`，颜色标识为：`orange`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(orange)`

> ###### ![orange](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-orange.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|橙色强调#(orange!)`，颜色标识为：`orange!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(orange!)`
...
>
> 这是一个 `#彩虹引用|黄色#(yellow)`，颜色标识为：`yellow`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(yellow)`

> ###### ![yellow](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-yellow.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|黄色强调#(yellow!)`，颜色标识为：`yellow!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(yellow!)`

---

> ###### ![lime](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-green.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|草绿色#(lime)`，颜色标识为：`lime`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(lime)`

> ###### ![lime](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-green.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|草绿色强调#(lime!)`，颜色标识为：`lime!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(lime!)`
...
>
> 这是一个 `#彩虹引用|绿色#(green)`，颜色标识为：`green`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(green)`

> ###### ![green](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-green.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|绿色强调#(green!)`，颜色标识为：`green!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(green!)`

---

> ###### ![aqua](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-cyan.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|青绿色#(aqua)`，颜色标识为：`aqua`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(aqua)`

> ###### ![aqua](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-cyan.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|青绿色强调#(aqua!)`，颜色标识为：`aqua!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(aqua!)`
...
>
> 这是一个 `#彩虹引用|青色#(cyan)`，颜色标识为：`cyan`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(cyan)`

> ###### ![cyan](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-cyan.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|青色强调#(cyan!)`，颜色标识为：`cyan!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(cyan!)`
...
>
> 这是一个 `#彩虹引用|蓝色#(blue)`，颜色标识为：`blue`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(blue)`

> ###### ![blue](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-blue.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|蓝色强调#(blue!)`，颜色标识为：`blue!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(blue!)`

---

> ###### ![sea](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-blue.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|海蓝色#(sea)`，颜色标识为：`sea`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(sea)`

> ###### ![sea](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-blue.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|海蓝色强调#(sea!)`，颜色标识为：`sea!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(sea!)`
...
>
> 这是一个 `#彩虹引用|紫色#(purple)`，颜色标识为：`purple`　　　ℹ️了解[颜色标识](#彩虹标签)
>
> `>(purple)`

> ###### ![purple](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-purple.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|紫色强调#(purple!)`，颜色标识为：`purple!`　　　ℹ️了解[颜色标识](#彩虹标签)
>
> `>(purple!)`

---

> ###### ![magenta](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-pink.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|洋红色#(magenta)`，颜色标识为：`magenta`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(magenta)`

> ###### ![magenta](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-pink.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|洋红色强调#(magenta!)`，颜色标识为：`magenta!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(magenta!)`
...
>
> 这是一个 `#彩虹引用|粉色#(pink)`，颜色标识为：`pink`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(pink)`

> ###### ![pink](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-pink.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|粉色强调#(pink!)`，颜色标识为：`pink!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(pink!)`

---

> ###### ![gold](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-brown.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|金色#(gold)`，颜色标识为：`gold`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(gold)`

> ###### ![gold](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-brown.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|金色强调#(gold!)`，颜色标识为：`gold!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(gold!)`
...
>
> 这是一个 `#彩虹引用|棕色#(brown)`，颜色标识为：`brown`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(brown)`

> ###### ![brown](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-brown.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|棕色强调#(brown!)`，颜色标识为：`brown!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(brown!)`
...
>
> 这是一个 `#彩虹引用|灰色#(gray)`，颜色标识为：`gray`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(gray)`

> ###### ![gray](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-gray.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|灰色强调#(gray!)`，颜色标识为：`gray!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(gray!)`
...
>
> 这是一个 `#彩虹引用|主题主色#(theme1)`，颜色标识为：`theme1`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(theme1)`

> ###### ![theme1](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-theme1.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|主题主色强调#(theme1!)`，颜色标识为：`theme1!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(theme1!)`
...
> ###### ![theme2](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-theme2.svg?mode=icon&fill=text&darksrc=invert) 彩虹引用 6 级标题
>
> 这是一个 `#彩虹引用|主题辅助色强调#(theme2!)`，颜色标识为：`theme2!`　　　ℹ️ 了解[颜色标识](#彩虹标签)
>
> `>(theme2!)`
...
[<kbd>![](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-back.svg?mode=icon&fill=text) 我要回到 快速入坑°内容导航</kbd>](#快速入坑°内容导航)
...
- 无序或有序列表内嵌的普通引用，会调整为「简约＋自适配宽度」样式，具体如下：

  > ###### ![theme2](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/qico-theme2.svg?mode=icon&fill=text&darksrc=invert) 普通引用 6 级标题
  >
  > 这是一个普通引用的内容
...
> > - 默认只对行数超过 1 行的代码块自动编号和添加题注，若希望调整该行数规则可通过[插件调校参数 `lmc`](#插件调校参数) 进行调整
...
**若你使用在[线插件方式](install.html#在线版插件)，可通过「[插件调校参数](#插件调校参数)」来动态指定任意 VLOOK 主题。**
...
[<kbd>![](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-back.svg?mode=icon&fill=text) 我要回到 快速入坑°主题与不杂项</kbd>](#快速入坑°主题与不杂项)
...
> 通过通过工具栏上的  ![字体主题](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-font-style.svg?mode=icon2x&fill=theme1&darksrc=invert)  按钮进行选择（快捷键操作：<kbd>A</kbd> )。
...
>---
>
>![蓝奏云](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/lanzou.png?mode=logo&srcset=@2x) … [下载字体▾](https://wwe.lanzoui.com/ieVDhj1aokj)　　　　　![百度网盘](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/baidupan.png?mode=logo&srcset=@2x) … [下载字体▾](https://pan.baidu.com/s/1gH5Hj-X3-LCaOLtN0AxLLw)（提取码`11ta`)
>
>`>(gray)`
...
> 可通过工具栏上的 ![Light Mode](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-light-mode.svg?mode=icon2x&fill=theme1&darksrc=invert) / ![Dark Mode](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-dark-mode.svg?mode=icon2x&fill=theme1&darksrc=invert) 按钮进行手动切换（快捷键操作：<kbd>D</kbd> )。
>
> 若希望文档在显示后强制指定为 Light / Dark Mode ，可通过「[插件调校参数](#插件调校参数)」来启用这一特性。
>
> 
>
...
> >    ```
> >
> > 3. 确定已按《安装与使用》中的 [Step 3•配置导出选项](index.html#step-3配置导出选项) 完成了「元标签代码」的安装，即「**偏好设置 ▸ VLOOK ▸ 在 &lt;head /&gt; 中添加**」已含有以下内容：
> >
> >    ```html
...
> - 通过工具栏上的 ![聚光灯](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-spotlight.svg?mode=icon2x&fill=theme1&darksrc=invert) 按钮打开、关闭（快捷键操作：<kbd>S</kbd> )；
...
[<kbd>![](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-back.svg?mode=icon&fill=text) 我要回到 快速入坑°演示与出版辅助</kbd>](#快速入坑°演示与出版辅助)
...
> 通过工具栏上的 ![激光笔](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-laser-pointer.svg?mode=icon2x&fill=theme1&darksrc=invert) 按钮打开「聚光灯」（快捷键操作：<kbd>P</kbd> )。
...
> - 鼠标悬停在表格后，通过右上角内容助手上的 ![表格阅读模式](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/icon-table-cross.svg?mode=icon2x&fill=theme1&darksrc=invert) 按钮打开、关闭（快捷键操作：<kbd>X</kbd> )；
...
>
> `#小提示#(orange)` 打印输出前，可根据提示选择 **显示** 或 **隐藏** 刮刮卡的内容。
...
完成加载处理后会自动关闭并显示文档，若希望自定义欢迎页的显示行为，可通过「[插件调校参数](#插件调校参数)」来指定。

若希望自定义欢迎页的显示内容，可以这样：

> 通过在文档最开始位置添加 [YAML Front Matter](install.html#进阶应用之-yamlL) 内容，并添加指定的 VLOOK™ 预置选项 `vlook-welcome`，举例如下：
>
...
> **识别链接类型**（鼠标悬停看看效果）：
>
> - 超链接协议类型：[**#页内链接**](#) / [**http**](http://) / [**https**](https://) / [**mailto**](mailto:)
...
|        el        | 调整文档的动效级别<br />默认为 `2`                           | `0` - 关闭动效<br />`1` - 标准动效<br />`2` - 增强动效（毛玻璃等） |
...
- 推荐使用以下浏览器及最新版本：
  ...
  ![Chrome](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/chrome.png?mode=icon2x) Google [**Chrome**](https://www.google.cn/chrome/)

  ![Edge](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/edge.png?mode=icon2x) Microsoft [**Edge**](https://www.microsoft.com/zh-cn/edge)

  ![Firefox](https://cdn.jsdelivr.net/gh/MadMaxChow/VLOOKres/pic/firefox.png?mode=icon2x) Mozilla [**Firefox**](https://www.mozilla.org/zh-CN/firefox/)

###### 动效

默认会启增强动效（包括毛玻璃），可以根据实际降低动效级别，可通过「[插件调校参数](#插件调校参数)」来进行调整
