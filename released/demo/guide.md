...
###### ~VLOOK™~<br>让你的 Markdown 有了新看^[wán]^法<br>──<br><u>快速参考手册<br>(Part.I)</u><br>`#最新版本|V18.0#`<br><br><br>**MAX°孟兆**<br>*COPYRIGHT © 2016-2023. MAX°DESIGN.*
...
以下是 VLOOK  的预置颜色色板，其中**大号加粗**的英文为对应的颜色标识，如：`Ye` `Aq`。

将在后续的[彩虹文本](#彩虹文本)、[渐变色文本](#渐变色文本)、[彩虹单元格](#彩虹单元格)、[彩虹标签](#彩虹标签)、[彩虹徽章](#彩虹徽章)、[彩虹引用](#彩虹引用)等文档排版特性中进行使用。



![VLOOK™ - Color Code 预置颜色标识](https://madmaxchow.gitee.io/vlookres/pic/vlook-color-code.png?darksrc=donate-wechat-dark.png&srcset=@2x#logo)
...
> 其中，
>
> `颜色`：`~ ~` 为 Typora 的下标格式。通过该方式指定标签使用的[预置颜色标识](#预置颜色标识)

> ###### 针对「整个段落」的文本着色
>
> ---
>
> 语法与左边的「针对指定格式的文本」是保持一致的，区别是在段落（或表格单元格）的最开始位置添加[预置颜色标识](#预置颜色标识)即可
>
...
|    颜色    | **颜色标识** | 单色渲染效果示例                   | 渐变渲染效果示例           |
| :--------: | :----------: | ---------------------------------- | -------------------------- |
|     红 Red     |      Rd~(Rd)~      | 警告、危险、关键事项、删除~(Rd)~  | 警告、危险、关键事项、删除~(Rd!)~ |
|     橙 Orange     |      Og~(Og)~      | 提醒、注意、修复~(Og)~         | 提醒、注意、修复~(Og!)~    |
|     黄 Yellow     |      Ye~(Ye)~      | 关注、优化、备忘、说明~(Ye)~   | 关注、优化、备忘、说明~(Ye!)~ |
|    柠绿 Lime    |      Lm~(Lm)~      | 提示、参考、新增~(Lm)~           | 提示、参考、新增~(Lm!)~   |
|     绿 Green     |      Gn~(Gn)~      | 提示、参考、新增~(Gn)~          | 提示、参考、新增~(Gn!)~    |
|    水绿 Aqua    |      Aq~(Aq)~      | 引用、公告~(Aq)~                 | 引用、公告~(Aq!)~          |
|     青 Cyan     |      Cy~(Cy)~      | 引用、公告~(Cy)~                 | 引用、公告~(Cy!)~          |
|     蓝 Blue     |      Bu~(Bu)~      | 信息、资讯~(Bu)~                 | 信息、资讯~(Bu!)~         |
|    海蓝 Sea    |      Se~(Se)~      | 信息、资讯~(Se)~                  | 信息、资讯~(Se!)~         |
|    藤紫 Vine    |      Vn~(Vn)~      | 信息、资讯~(Vn)~                | 信息、资讯~(Vn!)~          |
|     紫 Purple     |      Pu~(Pu)~      | 延伸、扩展、保留、备用~(Pu)~   | 延伸、扩展、保留、备用~(Pu!)~ |
|    玫红 Rose    |      Ro~(Ro)~      | 年轻、个性、女性~(Ro)~           | 年轻、个性、女性~(Ro!)~    |
|    粉红 Pink    |      Pk~(Pk)~      | 年轻、个性、女性~(Pk)~           | 年轻、个性、女性~(Pk!)~    |
|     金 Gold     |      Gd~(Gd)~      | VIP、金融、工程~(Gd)~            | VIP、金融、工程~(Gd!)~     |
|     棕 Brown     |      Bn~(Bn)~      | VIP、金融、工程~(Bn)~           | VIP、金融、工程~(Bn!)~     |
|     灰 Gray     |      Gy~(Gy)~      | 无效、暂缓、停用、结束~(Gy)~     | 无效、暂缓、停用、结束~(Gy!)~ |
|     黑 Black     |      Bk~(Bk)~      | 黑白、高对比~(Bk)~              | 黑白、高对比~(Bk!)~        |
|  主题主色 Theme1  |      T1~(T1)~      | 当前 VLOOK™ 主题的主色~(T1)~   | 当前 VLOOK™ 主题的主色~(T1!)~ |
| 主题辅助色 Theme2 |      T2~(T2)~      | 当前 VLOOK™ 主题的辅助色~(T2)~ | 当前 VLOOK™ 主题的辅助色~(T2!)~ |
...
`#VLOOK|👀#`~(T2)~ 全新的[预置颜色标识](#预置颜色标识)支持渐变色的，一肪相承，绝对能惊艳到你～
...
> 在单元格的最开始位置添加[预置颜色标识](#预置颜色标识)即可实现（[彩虹文本](#彩虹文本)的整个段落进行着色方式一致）

*==示例_彩虹单元格（单色）==*

|     ~(T1!)~彩虹单元格（单色示例）      |                ==                |                    ==                    |                ==                |
| :------------------------------------: | :------------------------------: | :--------------------------------------: | :------------------------------: |
|     ~(Rd)~红 Red<br>**Rd**~(Rd!)~      | ~(Og)~橙 Orange<br>**Og**~(Og!)~ |     ~(Ye)~黄 Yellow<br>**Ye**~(Ye!)~     | ~(Lm)~柠绿 Lime<br>**Lm**~(Lm!)~ |
|    ~(Gn)~绿 Green<br>**Gn**~(Gn!)~     | ~(Aq)~水绿 Aqua<br>**Aq**~(Aq!)~ |      ~(Cy)~青 Cyan<br>**Cy**~(Cy!)~      |  ~(Bu)~蓝 Blue<br>**Bu**~(Bu!)~  |
|    ~(Se)~海蓝 Sea<br>**Se**~(Se!)~     | ~(Vn)~藤紫 Vine<br>**Vn**~(Vn!)~ |     ~(Pu)~紫 Purple<br>**Pu**~(Pu!)~     | ~(Ro)~玫红 Rose<br>**Ro**~(Ro!)~ |
|    ~(Pk)~粉红 Pink<br>**Pk**~(Pk!)~    |  ~(Gd)~金 Gold<br>**Gd**~(Gd!)~  |     ~(Bn)~棕 Brown<br>**Bn**~(Bn!)~      |  ~(Gy)~灰 Gray<br>**Gy**~(Gy!)~  |
|    ~(Bk)~黑 Black<br>**Bk**~(Bk!)~     |                ==                |                    ==                    |                ==                |
| ~(T1)~主题主色 Theme1<br>**T1**~(T1!)~ |                ==                | ~(T2)~主题辅助色 Theme2<br>**T2**~(T2!)~ |                ==                |
...
> **语法：** `~(color)~` ，==必须在引用内中独占一行（首行、末尾均可）==
>
> 其中：
>
> `color`：指定标签显示对应的[预置颜色标识](#预置颜色标识)（除 `black` ）。

> > ###### 彩虹引用指定颜色的旧语法
> >
> > **语法：** `` ~color~ `` ，与新语法的主要区别是由行内代码改为下标，并采用与标签、徽章、刮刮卡统一的[预置颜色标识](#预置颜色标识)。
...
>
> 颜色==标识==为：`Rd`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> >  ```java
...
>
> 颜色==标识==为：`Og`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> >  ```java
...
>
> 颜色==标识==为：`Ye`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> >  ```java
...
>
> 颜色==标识==为：`Lm`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Gn`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Aq`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Cy`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Bu`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Se`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
> ###### ![sea](pic/qico-blue.svg?fill=text&darksrc=invert#icon) 彩虹引用 6 级标题
>
> 颜色==标识==为：`Vn`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Pu`　　　ℹ️了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Ro`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Pk`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Gd`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Bn`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`Gy`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
>
> 颜色==标识==为：`T1`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
> ###### ![Theme2](pic/qico-theme2.svg?fill=text&darksrc=invert#icon) 彩虹引用 6 级标题
>
> 颜色==标识==为：`T2`　ℹ️ 了解[颜色标识](#彩虹标签) [<kbd>了解更多 ![](pic/icon-forward.svg?fill=text#icon)</kbd>](#彩虹标签)
>
> > > ###### 更多信息
> >
> > ```java
...
> 其中，
>
> `title`：被折叠内容对应的标题，与折叠引子以「英文空格」进行分隔
>
> **使用了该语法后，其所在的引用就是被折叠的内容**
...
- 无序或有序列表内嵌的「[彩虹引用](#彩虹引用)」，会调整为「自适配宽度」样式，具体如下：
  ...
  > ###### ![Theme2](pic/qico-theme2.svg?fill=text&darksrc=invert#icon) 彩虹引用 6 级标题
  >
  > 这是一个 `#彩虹引用|主题辅助色#`~(T2)~ ，颜色==标识==为：`T2`　ℹ️ 了解[颜色标识](#彩虹标签)
  >
  > ~(T2)~
...
`#VLOOK|👀#`~(T2)~ 对 Markdown 行内代码（Code）语法 `` ` ` `` 和下标语法 `~ ~` 进行了扩展，满足实用的「标签」和「徽章」排的版需求！同时还支持一键复制内容、复制为 Markdown（点击并按住 <kbd>Alt</kbd> 或  <kbd>⌥ option</kbd> ）。
...
> `标签`：徽章内容，前后由 `#` 符号包裹
>
> `颜色`：可选。`~ ~` 为 Typora 的下标格式。通过该方式指定标签使用的[预置颜色标识](#预置颜色标识)（支持线框/色块风格），不指定则默认为 `T2`
...
| :--------------: | ------------------------------- | -------------------------- | ------------------------- |
|       Rd        | `` `#标签 Rd#`~(Rd)~ ``       | 警告、危险、关键事项、删除 | `#标签 Rd#`~(Rd)~      |
...
|      Bk      | `` `#标签 black#`~(Bk)~ ``   | 黑白、高对比               | `#标签 black#`~(Bk)~   |
...
>
> `颜色`：可选。`~ ~` 为 Typora 的下标格式。通过该方式指定标签使用的[预置颜色标识](#预置颜色标识)，不指定则默认为 `(Gy)` 。颜色标识后添加 `!` 则使用强调样式
...
| 预置颜色标识 | 语法示例                                     | 应用场景建议               | 渲染效果                               |
| :----------: | -------------------------------------------- | -------------------------- | -------------------------------------- |
|     Rd      | `` `#标题|徽章内容 {{变量 1}}#`~(Rd)~ ``    | 警告、危险、关键事项、删除 | `#标题|徽章内容 {{变量 1}}#`~(Rd)~    |
...
|     Cy     | `` `#标题|徽章内容 {{变量 1}}#`~(Cy)~ ``   | :                          | `#标题|徽章内容 {{变量 1}}#`~(Cy)~   |
|     Bu     | `` `#标题|徽章内容 {{变量 1}}#`~(Bu)~ ``   | 信息、资讯                 | `#标题|徽章内容 {{变量 1}}#`~(Bu)~   |
|     Se     | `` `#标题|徽章内容 {{变量 1}}#`~(Se)~ ``    | :                          | `#标题|徽章内容 {{变量 1}}#`~(Se)~    |
|    Vn    | `` `#标题|徽章内容 {{变量 1}}#`~(Vn)~ ``  | :                          | `#标题|徽章内容 {{变量 1}}#`~(Vn)~  |
...
| 预置颜色标识 | 语法示例                            | 应用场景建议               | 渲染效果                     |
| :----------: | ----------------------------------- | -------------------------- | ---------------------------- |
|     Rd!     | `` `#标题|强调样式#`~(Rd!)~ ``     | 警告、危险、关键事项、删除 | `#标题|强调样式#`~(Rd!)~    |
...
`#VLOOK|👀#`~(T2)~ 无须任何设置，即可自动对文档内所有「Mermaid 图表」进行统一的自动编号，编号内容举例：`插图 1` `插图 2` ，命名和编号与[插图](#插图自动编号与题注)一致。同时，还增强了在演示时的辅助特性 … [了解详情 ❯](#Mermaid 样式优化与扩展)
...
> - `隐藏文本`：被刮刮卡遮盖的原始信息
> - `颜色`：可选。`~ ~` 为 Typora 的下标格式。通过该方式指定刮刮卡使用的[预置颜色标识](#预置颜色标识)，不指定则默认为 `(Gy)` 。预置颜色标识与「[彩虹标签](#彩虹标签)」的颜色标识基本保持一致（无 `black` ，且不支持色块风格标识 ）
>
> `#💡|小提示#`~(Og!)~
>
