...
###### <sub>VLOOK™</sub><br />让你的 Markdown 有了新`{看}(wán)`法<br />──<br /><sup>快速参考手册</sup><br />`#最新版本|V12.0#(theme2)`<br /><br /><br />**MAX°孟兆**<br />*COPYRIGHT © 2016-2021. MAX°DESIGN.*
...
| ------------------------------------------------------------ | :--: | :----------------------------------------------------------- |
| **表格**                                                     |      | **图片、图标、插图**                                         |
| - 横向、纵向**合并**指定单元格 … [more ❯](#表格单元格合并)   |  :   | - 为插图进行自动编号、**添加题注** … [more ❯](#插图自动编号与题注) |
| - 将**整列**设置为加粗、高亮、下划线、斜体格式 … [more ❯](#表格列格式：常规) |  :   | - 指定插图的**对齐方式** … [more ❯](#插图对齐方式)           |
| - 将整列设置为**数值**、**百分比**、**货币格式** … [more ❯](#表格列格式：数值、百分数、货币) |  :   | - 让图片在高分屏下自动适配显示**高清**的版本（2x、3x） … [more ❯](#图片适配高分屏) |
| - 将整列设置为**复选框**格式，并能指定某单元格为「选中」 … [more ❯](#表格列格式：复选框) |  :   | - 让图片自动适配 **Dark Mode**  … [more ❯](#图片适配 Dark Mode) |
| - 给每个表格进行自动编号、**添加题注** … [more ❯](#表格自动编号与题注) |  :   | - 添加的**小图标**、普通图片，不想以插图的形式显示 … [more ❯](#图片显示版式) |
| - 给表格的**每一行**都进行**自动编号** … [more ❯](#表格逐行自动编号) |  :   | - **一行多图**自动换行后样式**适配** … [more ❯](#一行多图适配) |
| - 对同属一类内容的**表格行**进行分组、分类，并能**自动折叠** … [more ❯](#表格行分组折叠) |  :   | - 让图标的颜色与周围的元素更搭更和谐 … [more ❯](#图片颜色替换) |
| - 指定表格可**重复显示表头** … [more ❯](#表格重复显示表头)   |  :   | - 为图片添加工程风的网格背景 … [more ❯](#图片网格背景)       |
| - 在表格中指示当前行 … [more ❯](#表格当前行指标器)           |  :   | - 图片边缘能**留白** … [more ❯](#图片边缘留白)               |
|                                                              |  ==  | ==                                                           |
| **标签、标签组**                                             |      | **流程图、状态机图、顺序图**                                 |
| - 将特定关键字设置为**标签**样式 … [more ❯](#彩虹标签)       |  :   | - **扩展** Mermaid 的**图表**，并优化了**样式** … [more ❯](#°Mermaid) |
| - 将特定的**二元组**内容设置为**标签组**样式 … [more ❯](#药丸标签) |  :   |                                                              |
|                                                              |  ==  | ==                                                           |
| **音频、视频、注音**                                         |      | **引用、引用块**                                             |
| - 为文档添加音频内容 … [more ❯](#°音频)                      |  :   | - **重要**的引用指定使用**突出**的样式（如：重要提示） … [more ❯](#彩虹引用) |
| - 为文档添加视频内容 … [more ❯](#°视频)                      |  :   | - **分栏**（双栏 / 三栏 / 四栏）方式显示引用块 … [more ❯](#分栏引用) |
| - 要给生`{僻}(pì)`字，或东亚小语种**添加注音** … [more ❯](#°注音) |  :   | - **引用**的内容支持**自定义折叠** … [more ❯](#折叠吧引用)   |
|                                                              |  ==  | ==                                                           |
| **代码块**                                                   |      | **任务列表**                                                 |
| - 给每个代码块进行自动编号、**添加题注** … [more ❯](#代码块自动编号与题注) |  :   | - 任务列表的序号，给你找回来了 … [more ❯](#任务列表自动编号) |
| - 代码块高亮当前行、一键复制代码 … [more ❯](#代码块增强)     |  :   | - 对于多级任务列表，「**任务组完成状态**」能自动识别了 … [more ❯](#任务组完成状态) |
...
| **内容导航工具** | 适用场景                                                     |
| :--------------: | ------------------------------------------------------------ |
|     导航中心     | 适用于按目录、分类内容索引（目录/插图/表格/多媒体/代码块）进行**全文档**跨章节的快速定位 … [more ❯](#导航中心) |
|     逐章导航     | 适用于前后章节相关度高，**按章节**顺序一章一章进行浏览 … [more ❯](#逐章导航) |
|     逐段导航     | 适用于文档评审或演示，**按段落**一段一段进行聚焦式浏览 … [more ❯](#段落导航) |
|     插图导航     | 适用于**按图索骥**，快速浏览文档中的插图，以及快速**定位**到文档中对应的位置 … [more ❯](#插图导航) |
|     文库导航     | 适用于将相关话题、内容的文档简介、链接，通过一个独立页面进行快速浏览和跳转 … [more ❯](#文库导航) |
|     脚注导航     | 直接在当面位置进行显示脚注，避免来回跳转 … [more ❯](#脚注导航) |
...
| **演示辅助工具** | 适用场景                                                     |
| :--------------: | ------------------------------------------------------------ |
|      聚光灯      | 对屏幕内容的局部进行视觉**聚焦** … [more ❯](#聚光灯)         |
|      激光笔      | 对文档内容进行**精确指向** … [more ❯](#激光笔)               |
|   表格阅读模式   | 针对表格内容，特别是行、列关联性强的**表格**，快速识别**纵横关联**的单元格序列 … [more ❯](#表格阅读模式) |
|      画中画      | 插图/表格/代码块 一键开启「画中画」浮动显示，方便在文档相关内容间对比 … [more ❯](#画中画) |
|      刮刮卡      | - 制作培训、猜迷互动类文档时，希望将预置的**答案隐藏**起来 … [more ❯](#刮刮卡) |

![VLOOK™ 的「出版辅助」工具及适用场景]

| **出版辅助工具** | 适用场景                                                     |
| :--------------: | ------------------------------------------------------------ |
| 章节标题自动编号 | 对文档内的章节标题进行**自动分级编号** … [more ❯](#章节标题自动编号) |
| 自定义欢迎页内容 | 对文档在加载过程中显示欢迎语内容 … [more ❯](#自定义欢迎页内容) |
|   文档内容检查   | 对文档的**图片、音频、视频、页内链接**等内容进行检查并提示 … [more ❯](#内容失效检查) |
|  链接识别与优化  | **自动识别**并提示文档内的**链接**类型，并针对外链进行了**优化** … [more ❯](#链接识别与优化) |
|   适配打印输出   | 在打印前将文档的样式调整和简化，以**适配打印**输出的应用需要 … [more ❯](#适配打印输出) |
...
| -------------------------------------------------------- | ---- | ------------------------------------------------------------ |
| **模板主题、字体主题**                                   |      | **不杂项**                                                   |
| 为不同类型的文档设置不同的模板主题 … [more ❯](#模板主题) | :    | 通过启动参数，自定义你的文档表现 … [more ❯](#启动参数)       |
| 为文档选择不同的字体主题 … [more ❯](#字体主题)           | :    | 长内容会自动折叠了，长插图、长表格、长代码 … [more ❯](#长内容自动折叠) |
| Dark Mode is NOW … [more ❯](#Dark Mode)                  | :    | 支持快捷键操作 … [more ❯](#快捷键操作汇总)                   |
| 为文档添加封面、封底 … [more ❯](#封面、封底)             | :    | 支持 Typora 编辑时的特性增强 … [more ❯](#Typora 编辑时特性增强) |
| 更美观的公式 … [more ❯](#更美观的公式)                   | :    | 支持[跨平台、动效](#跨平台、动效支持)（如：毛玻璃） [emoji](#emoji 支持)、[国际化](#国际化支持) |
...
|    green     | `` `#标签 green#(green)` ``   | 提示、参考、新增             | `#标签 green#(green)`   |
|     cyan     | `` `#标签 cyan#(cyan)` ``     | 引用、公告                   | `#标签 cyan#(cyan)`     |
|     blue     | `` `#标签 blue#(blue)` ``     | 信息、资讯                   | `#标签 blue#(blue)`     |
|    purple    | `` `#标签 purple#(purple)` `` | 延伸、扩展、保留、备用       | `#标签 purple#(purple)` |
|     pink     | `` `#标签 pink#(pink)` ``     | 年轻、个性、女性             | `#标签 pink#(pink)`     |
|    brown     | `` `#标签 brown#(brown)` ``   | 稳定、金融、工程             | `#标签 brown#(brown)`   |
...
> 其中，
>
> `tag1` `tag2`：分别为标签组和标签内容，由 `#` 包裹，并以英文竖线符号 `|` 作为分隔
>
> `color`：可选。指定标签显示对应的预置颜色标识，不指定则默认为 `(red)`
...
| 预置颜色标识 | 语法示例                             | 应用场景建议                 | 渲染效果                       |
| :----------: | ------------------------------------ | ---------------------------- | ------------------------------ |
|     red      | `` `#标签组|内容 red#(red)` ``       | 警告、危险、关键事项、删除   | `#标签组|内容 red#(red)`       |
|    orange    | `` `#标签组|内容 orange#(orange)` `` | 提醒、注意、修复             | `#标签组|内容 orange#(orange)` |
|    yellow    | `` `#标签组|内容 yellow#(yellow)` `` | 关注、优化、备忘、说明       | `#标签组|内容 yellow#(yellow)` |
|    green     | `` `#标签组|内容 green#(green)` ``   | 提示、备忘、参考、新增       | `#标签组|内容 green#(green)`   |
|     cyan     | `` `#标签组|内容 cyan#(cyan)` ``     | 引用、公告                   | `#标签组|内容 cyan#(cyan)`     |
|     blue     | `` `#标签组|内容 blue#(blue)` ``     | 信息、资讯                   | `#标签组|内容 blue#(blue)`     |
|    purple    | `` `#标签组|内容 purple#(purple)` `` | 延伸、扩展、保留、备用       | `#标签组|内容 purple#(purple)` |
|     pink     | `` `#标签组|内容 pink#(pink)` ``     | 年轻、个性、女性             | `#标签组|内容 pink#(pink)`     |
|    brown     | `` `#标签组|内容 brown#(brown)` ``   | 稳定、金融、工程             | `#标签组|内容 brown#(brown)`   |
|     gray     | `` `#标签组|内容 gray标签#(gray)` `` | 无效、暂缓、停用、结束       | `#标签组|内容 gray#(gray)`     |
|    theme1    | `` `#标签组|内容 theme1#(theme1)` `` | 使用当前 VLOOK™ 主题的主色   | `#标签组|内容 theme1#(theme1)` |
|    theme2    | `` `#标签组|内容 theme2#(theme2)` `` | 使用当前 VLOOK™ 主题的辅助色 | `#标签组|内容 theme2#(theme2)` |
...
```C++
// ================================================== ////            VLOOK™ 支持对长代码块进行自动折叠           //// ================================================== ///** * 快速排序 * @param arr 排序的目标数据 * @param left 排序范围左值 * @param right 排序范围右值 */void quickSort(int arr[], int left, int right) {    int i = left, j = right, tmp, pivot = arr[(left + right) / 2];      /* partition */    while (i <= j) {        while (arr[i] < pivot)            i++;        while (arr[j] > pivot)            j--;        if (i <= j) {            tmp = arr[i];            arr[i] = arr[j];            arr[j] = tmp;            i++;            j--;        }    };    /* recursion */    if (left < j)        quickSort(arr, left, j);    if (i < right)        quickSort(arr, i, right);}// ================================================== //
```
...
| 表格列格式(数值/百分数/货币) … [more ❯](#表格列格式：数值、百分数、货币) |                DIN                 |     [x]     |            |   [x]    |           |
...
```html
<meta name="vlook-query" content="${vlook-query}"><meta name="vlook-welcome" content="${vlook-welcome}">
```
