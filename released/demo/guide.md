...
![VLOOK™ - Color Code 预置颜色标识](https://madmaxchow.gitee.io/vlookres/pic/vlook-color-code.png?darksrc=donate-wechat-dark.png&srcset=@2x#logo#center)

**（ `~内容~` 为 Typora 的下标格式，被 VLOOK™ 主题识别的预置颜色标识都会以特殊样式显示，以区别于普通下标样式）**
...
> 其中：
>
> `颜色`：指定该引用使用的[预置颜色标识](#预置颜色标识)（除 `black` ），在被正常识别后会在颜色标识前出现彩虹引用标识「**>**~(T2)~」
...
> `标签`：徽章内容，前后由 `#` 符号包裹
>
> `颜色`：可选。指定该标签使用的[预置颜色标识](#预置颜色标识)（支持常规/强调风格），不指定则默认为 `T2`
...
>
> `颜色`：可选。指定该徽章使用的[预置颜色标识](#预置颜色标识)，不指定则默认为 `(Gy)` 。颜色标识后添加 `!` 则使用强调样式
...
> - `隐藏文本`：被刮刮卡遮盖的原始信息
> - `颜色`：可选。`~ ~` 为 Typora 的下标格式。通过该方式指定刮刮卡使用的[预置颜色标识](#预置颜色标识)，不指定则默认为 `(Gy)` 。预置颜色标识与「[彩虹标签](#彩虹标签)」的颜色标识基本保持一致（无 `black` ，且不支持强调风格标识 ）
>
> `#💡|小提示#`~(Og!)~
>
