...
###### ~VLOOK™~<br>让你的 Markdown 有了新看^[wán]^法<br>──<br><u>快速参考手册<br>(Part.II)</u><br>`#最新版本|V18.0#`<br><br><br>**MAX°孟兆**<br>*COPYRIGHT © 2016-2023. MAX°DESIGN.*
...
`guide.md#微信支付|MaxChowguide.md#`~(Gn!)~
...
> - 给生僻^[pì]^字，或东亚小语种添加**注音** [❯❯](guide.md#°注音)
...
> ###### 🎁 私人定制
>
> 目前 VLOOK™ 项目也现已开放主题的 **私人定制** 服务 `#微信|MaxChow#`~(Gn)~ ，定制样品示意如下：
>
> ~(Bn!)~
...
> 如：`###### 这是封面标题`
>
> `#⚠️|注意#`~(Rd!)~ 如文档使用 YAML Front Matter，则添加到其后则可
>
> ~(T1)~
...
> 以下的内容复制功能均支持「**复制为 Markdown 格式**~(T1T2)~」，点击前同时按下 <kbd>Alt</kbd>  或  <kbd>⌥ option</kbd> 即可实现。
...
|   vlook-image-host    | 默认图床地址，如：`https://xxx.abc.com/images` | 目前主要针对 Safari 打开本地 HTML 的文件，且指定[图片颜色替换](guide.md#图片颜色替换)时使用 |
...
|       tag        | 调整[彩虹标签](guide.md#彩虹标签)未指定颜色时默认使用的颜色标识<br>默认为 `T1` | 对应的预置颜色标识，[详见这里](guide.md#彩虹标签)            |
|      badge       | 调整[彩虹徽章](guide.md#彩虹徽章)未指定颜色时默认使用的颜色标识<br>默认为 `Gy` | 对应的预置颜色标识，[详见这里](guide.md#彩虹徽章)            |
|      quote       | 调整普通「引用」默认自动转换为指定颜色标识的[彩虹引用](guide.md#彩虹引用)<br>默认为 `T1` | 对应的预置颜色标识，[详见这里](guide.md#彩虹引用)            |
|       coat       | 调整[刮刮卡](guide.md#°刮刮卡)未指定颜色时默认使用的颜色标识<br>默认为 `Gy` | 对应的预置颜色标识，[详见这里](guide.md#°刮刮卡)             |
...
> > 
> >
> > `#⚠️|注意#`~(Rd)~ 若图片 URL 中含有锚点内容，如 `xxx.png#icon2x` ，请在锚点前添加 URL 参数。
> >
