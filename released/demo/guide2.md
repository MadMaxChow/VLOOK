...
vlook-query: el=2&ws=auto&lmc=2&tag=gray&badge=theme1
vlook-doc-lib: vlook-lib.html#abc
---

###### <sub>VLOOK™</sub><br>让你的 Markdown 有了新`{看}(wán)`法<br>──<br><sup>快速参考手册<br><sup>(Part.II)</sup></sup><br>`#最新版本|V17.0#`<br><br><br>**MAX°孟兆**<br>*COPYRIGHT © 2016-2023. MAX°DESIGN.*
...
> 内容复制功能还支持复制为 Markdown 格式，操作前按下 <kbd>Alt</kbd>  或  <kbd>⌥ option</kbd> 即可实现。

- **标签：**点击即复制对应的标签内容，如：`#这是标签，点击复制#`
- **徽章：**点击徽章标题、徽章内容，即复制对应的内容，如：`#这是徽章标题|这是徽章内容#`
- **表格：**一键复制表格全部内容，[点这里找个表格试试 ❯](#表格阅读模式)
- **插图：**复制图片的链接，[点这里去找张图片试试 ❯](#示例_长图片、双题注)
- **代码：**点击即复制对应的代码内容，如：`这是代码，点击复制`
- **代码块：**一键复制全部代码行内容，如：
...
|       tag        | 调整[彩虹标签](guide.md#彩虹标签)未指定颜色时默认使用的颜色标识<br>默认为 `theme1` | 对应的预置颜色标识，[详见这里](guide.md#彩虹标签)            |
|      badge       | 调整[彩虹徽章](guide.md#彩虹徽章)未指定颜色时默认使用的颜色标识<br>默认为 `gray` | 对应的预置颜色标识，[详见这里](guide.md#彩虹徽章)            |
|      quote       | 调整普通「引用」默认自动转换为指定颜色标识的[彩虹引用](guide.md#彩虹引用)<br>默认为 `theme1` | 对应的预置颜色标识，[详见这里](guide.md#彩虹引用)            |
|       coat       | 调整[刮刮卡](guide.md#°刮刮卡)未指定颜色时默认使用的颜色标识<br>默认为 `gray` | 对应的预置颜色标识，[详见这里](guide.md#°刮刮卡)             |
