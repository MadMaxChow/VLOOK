...
- [攻略 ★ 探索](guide3.html?target=_self "内容导航/历史/上次阅读、内容助手/复制/全屏/换行/十字光标/阅读/画中画、演示辅助/激光笔/聚光灯/段落漫游/宽度适配/宁静视图、出版辅助/标记不发布/发布为PDF/导出为PDF/社交分享/链接地图/长内容折叠/链接检查/链接识别、外观/主题/字体/Light/Dark、自定义/预置选项/调校参数、快捷键、……")
...
<u>通过右下角状态栏上的  ![字体风格](pic/icon-font-style.svg?darksrc=invert#icon)  按钮进行选择<br>也支持通过「[定制主题](#定制模板主题)」进行预置指定</u>
...
<u>可通过右下角状态栏上的 ![Auto Mode](pic/icon-auto-mode.svg?darksrc=invert#icon) / ![Light Mode](pic/icon-light-mode.svg?darksrc=invert#icon) / ![Dark Mode](pic/icon-dark-mode.svg?darksrc=invert#icon) 按钮进行手动切换（快捷键 <kbd>A</kbd> )</u>
...
支持移动端的浏览器顶部的主题色（品牌色）特性，获得更好友、一致的浏览体验。
...
> - 在文档中配置了「文库」后（或文档中含有文库类型的链接时），可通过导航中心内显示「文库」条目；
> - 支持为「文库」配置了多条链接；
...
> - 通过工具栏上的 ![聚光灯](pic/icon-spotlight.svg?darksrc=invert#icon) 按钮打开、关闭（快捷键 <kbd>O</kbd> )；
...
**当你需要沉浸式的阅读体验时，可以通过快捷键 <kbd>Z</kbd> 一键进入「宁静视图」。**

<u>开启后将隐藏在工具栏、导航中心、状态栏等元素。可再次按下 <kbd>Z</kbd> 键或 <kbd>ESC</kbd> 退出「宁静视图」。</u>



> [!TIP]
>
> 在「宁静视图」下如需使用「[激光笔](#激光笔)、[聚光灯](#聚光灯)、[段落漫游](#段落漫游)」等演示辅助工具，可直接通过对应的快捷键方式进行启用。
...
# 示例 1
vlook-footer: {{title}}<br><br>{{author}}<br>{{date}}

# 示例 2
vlook-footer: <a href="xxx.com">更多精彩内容</a><br><br>{{copyright}}
---
```
...
**在加载过程中支持显示在[定制主题](vip.md)时指定的默认占位图。**
...
| 分类                                      | 功能                                        |            快捷键            |
| :-----------------------------------------: | :--------------------------: | :--------------------------: |
| 常用                      | 切换 Light / Dark Mode  |         <kbd>A</kbd>         |
