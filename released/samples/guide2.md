...
> - 自动对文档内容进行分类索引，分类包括：==插图==、==表格==、==代码块==、==公式==、==多媒体==
> - 目录大纲、文库、分类索引条目均支持按关键字进行搜索
> - 自动记录跳转位置，可以随时通过可通过右上角的 ![导航历史](pic/icon-history.svg?fill=theme1&darksrc=invert#icon) 导航历史按钮快速回跳（快捷键 <kbd>H</kbd> )
>

> [!IMPORTANT]
>
> - 基于 Typora 生成的目录大纲，请在文档中「VLOOK™ 的[封面](#封面与封底)」后添加目录标记 `[TOC]` ；
> - 若文档无目录大纲和内容索引，导航中心会默认收起。
