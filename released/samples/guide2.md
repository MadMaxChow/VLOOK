...
*VLOOK`A`*_~T2~_ 点击工具栏上的逐段导航按钮 ![段落导航](pic/icon-paragraph-nav.svg?darksrc=invert#icon) （或快捷键）就可以进入「段落导航」模式了（致敬 VI)。
...
> **组合键触发**
>
> 先按下 <kbd>Alt</kbd>  或  <kbd>⌥ option</kbd> ，并点击 ![复制](pic/icon-copy.svg?darksrc=invert#icon) 按钮即可

> **二次点击触发**
>
> 直接点击 ![复制](pic/icon-copy.svg?darksrc=invert#icon) 按钮后，会自动激活「复制为 Markdown」![复制](pic/icon-copy-as-markdown.svg?darksrc=invert#icon) ，在 2 秒内再点击一次即可（主要满足在移动设备上访问时使用)
