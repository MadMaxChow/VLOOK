...
目前提供了「小封面」和「大封面」两种选择，根据实际需要进行选用：
...
>
> > 简介文档使用的封面为「大封面」，可打开[这个链接](index.md)看看效果
>
> _~T1!~_

## 小封面

> **适用范围 ••• *`编辑时`× 不支持*_~Gy~_  *`导出 HTML`✓ 支持*_**
...
*==小封面支持设置并显示以下 YAML 的字段==*

```yaml
---
title: 标题内容
author: 作者
date: 发布日期。如：2025-11-11
version: 版本信息。如：V1.0.1
copyright: 版权信息。如：Copyright © 1981 示例公司. 保留所有权利.
rights: 权利说明。如：本作品采用 MIT 许可证授权，详见 LICENSE 文件
tags: 标签内容，多个之间用英文逗号分隔。如：标签1, 标签2, 标签3
---
```
...
> - 可通过「[插件调校参数](guide3.md#插件调校参数)」 `capnum` 调整题注是否启用自动编号；
...
> ###### 关于图片题注的进阶用法
>
> 1. **不将图片题注内容，只显示题注分类（如：图 1、图 2 等）**
>    - 可通过「[插件调校参数](guide3.md#插件调校参数)」 `capmini` 来调整只显示题注分类
> 2. **如图片没有指定题注内容时，也希望显示题注分类**
>    - 可通过「[插件调校参数](guide3.md#插件调校参数)」 `capblank` 来调整在无题注内容时强制显示题注分类
