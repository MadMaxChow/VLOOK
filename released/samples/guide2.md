...
> - 鼠标悬停在插图后，通过右上角内容助手上的 ![画中画](pic/icon-full-screen.svg?fill=theme1&darksrc=invert#icon) 按钮打开，点击插图浏览器中的插图，也能直接跳转到文档中对应的位置。
...
> - 文档中配置了「文库」，或在文档中含有文库链接时，在工具栏、目录大纲下方都会显示「文库」入口 ![字体主题](pic/icon-doc-lib.svg?fill=theme1&darksrc=invert#icon) ；
...
> - 鼠标悬停在表格后，通过右上角内容助手上的 ![表格阅读模式](pic/icon-table-cross.svg?fill=theme1&darksrc=invert#icon) 按钮打开、关闭（快捷键操作：<kbd>X</kbd> )；
...
> - 可单独对不同的表格指定不同的换行版式，通过右上角内容助手上的 ![画中画](pic/icon-table-wrap.svg?fill=theme1&darksrc=invert#icon) 手工切换为「自动换行」或「不换行」。
...
> - 鼠标悬停在插图、表格、代码块后，通过右上角内容助手上的 ![画中画](pic/icon-pic-in-pic.svg?fill=theme1&darksrc=invert#icon) 按钮一键开启；
...
> **音频、视频源失效**
>
> 自动识别无法加载的[音频](guide.md#音频)、[视频](guide.md#视频)，并进行提示，示例：
>
...
> [!TIP]
>
> 存在需要关注或处理建议的异常结果时，会在文档的右下角显示 ![](pic/link-warning.png#icon) 或 ![](pic/link-error.png#icon) 图标提示，点击可查看具体的异常信息。
...
*Markdown 粉`⬇️`*「**希望在国家公祭日，或其它指定的日期进行浏览时，能自动变为灰色显示～**」
...
*📒`参考`*_~Gn!~_ 更多脚本化图表样式，以及结合 VLOOK™ 相关的脚本编写规范内容详见《[脚本化图表 for Markdown](chart.md)》。
