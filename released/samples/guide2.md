...
*Markdown 粉`Q`*「**用 Markdown 写文档，怎样才能以分段式的结构组织或呈现相关性较强的内容？**」

*VLOOK`A`*_~T2~_ 在前面提到[标签](#标签)的基础上，在标签的前或后添加需要分段的内容就可以了，AMAZING！
...
>
> 1. 先按「[标签](#标签)」完成设置后；
> 2. 在代码符号 `` ` `` 包裹的内容前面，或后面添加徽章标题，或内容2即可
>
...
*Markdown 粉`Q`*「**按大纲 / 索引进行导航是文档中最常用的导航方式，适用于内容层次明确，全文档跨章节的快速定位**」

*VLOOK`A`*_~T2~_ VLOOK™ 对导出的 HTML 自动根据其内容生成目录大纲、分类内容索引。
...
- 复制「[徽章](#徽章)」
  ...
  点击徽章标题/内容/内容2，即复制其自身及右侧的内容，如：*徽章标题`内容1`内容2*
...
> 鼠标悬停在「表格、代码块」后，通过右上角内容助手上的 ![画中画](pic/icon-wrap-unwrap.svg?fill=theme1&darksrc=invert#icon) 手工切换为「自动换行」或「不换行」
...
*Markdown 粉`Q`*「**希望插图、表格、代码块也能像视频那样，可以设置为「画中画」的浮动显示模式，方便在文档相关内容间对比**」
...
*VLOOK`A`*_~T2~_ Markdown 的「删除线」和「标题」一拍即合，优雅地轻松驾驭发布范围～
...
>
> - 指定的 1 列或多列
> - 参考[表格列格式](guide.md#表格列格式)，将列头设置为删除线格式即可
...
*VLOOK`A`*_~T2~_ 提供了两套自定义的字体主题，可根据个人喜好进行选用。
...
```markdown
###### ~VLOOK™~<br>让你的 Markdown 有了新看_^wán^_法<br>──<br><u>快速参考手册<br>(Part.II)</u><br>*最新版本`V26.0`*<br><br><br>**MAX°孟兆**<br>*COPYRIGHT © 2016-2024. MAX°DESIGN.*
```
