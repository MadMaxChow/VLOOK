---
title: 快速参考手册 • 基础 - VLOOK™ - 优雅好用的 Typora 主题与增强插件
author: MAX°孟兆
...
###### ~VLOOK™~<br>让你的 Markdown 有了新看_^wán^_法<br>──<br><u>快速参考手册 • 基础</u><br>*最新版本`V28.1`2025-01-18*<br><br>**MAX°孟兆**<br>*COPYRIGHT © 2016-2025. MAX°DESIGN.*
...
<u>VLOOK™ 对「高亮、加粗、斜体、上标、下标」等常规格式进行扩展和组合后，提供多种个性实用的排版能力</u>
...
<u>VLOOK™ 对 Markdown 的一级、六级标题进行了延伸，用于提供了一些实用且独特的排版能力</u>
...
> **适用范围 ••• *`编辑时`✓ 适用*_~Gn~_  *`导出 HTML`✓ 适用*_~Gn~_**
>
> 满足对**整个段落内容**居中、稍微突显，与后续内容有视觉区隔，但又有所关联的需求。示例如下：

<u>～这是一段**稍微突显**，与后续内容有**视觉区隔**，但又有所**关联**的内容。**字号加大**、**居中**，其他不变～</u>

<sub>（以上段落下方的分隔线样式，支持通过 [定制主题](vip.md) 调整样式）</sub>

## 段落引用

> **适用范围 ••• *`编辑时`✓ 适用*_~Gn~_  *`导出 HTML`✓ 适用*_~Gn~_**
>
> 满足对**整个段落内容**以更个性、更突显的引用样式，并与前/后内容有视觉区隔的需求。示例如下：

==～这是一段 **更个性、更突显**_~RdPuRo~_ ，与前/后内容有<u>视觉区隔</u>的内容。<u>更大字号</u>、<u>居中</u>，其他不变～==

<sub>（以上段落首尾的引用样式，支持通过 [定制主题](vip.md) 调整样式）</sub>
...
*VLOOK`A`*_~T2~_ 无须任何设置，即可自动对文档内所有「表格」进行统一的自动编号，编号举例： `表 1` `表 2.3-2`
...
> **单题注**
>
> 实现为表格在自动编号之后，添加指定的题注，如：`表 1-1. 2020 年全球经济体 GDP 排名`
>
> _~T1!~_
---
...
> 以下表格在指定 `capauto` 为 `on` ，且不通过题注语法指定题注内容时，会自动生成题注内容
...
<u>表格前添加一个 `6 级` 的标题（快捷键操作：<kbd>Cmd / Ctrl</kbd> + <kbd>6</kbd> ），即可为表格的每一行进行自动编号。</u>
...
*VLOOK`A`*_~T2~_ 对于独占一行的图片，或强制指定 [显示版式](#图片显示版式) 为插图的，VLOOK™ 会将以其「插图[^插图]」的形式处理，如统一自动编号和题注，编号举例： `图 1` `图 2.3-2`
...
> - 图片添加 `#border` 后，图片宽度较小无法完全填充分栏空间时，可额外添加 `#fill` 参数，查看示例 [❯❯](vip.html#排版编辑服务)
...
>
> 若希望获得更具个性的标题样式（如：颜色、渐变色、边框、留白等）、自定义自动编号格式，可以订阅 ==定制主题服务== 。
>
> 
>
