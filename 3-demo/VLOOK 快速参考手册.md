...
`#最新版本|V9.17#msg`    `#适用编辑器|Typora#notice`    `#开源协议|MIT#`   `#开发语言|JavaScript HTML/CSS/LESS#spare`    `#官网|https://github.com/madmaxchow/VLOOK#tips`
...
- 团队或项目的文档须统一模板化排版与输出，并能随时切换不同模板
- 只关注文档内容，排版工作希望能自动化完成
- 以网络作为主要的发布渠道，主要发布为 HTML 格式，同时能发布为 PDF 等其它常用格式
- 减少在文档（如 Word）、制图（如 Visio）等软件工具方面的购买支出
- 需要支持跨平台（Windows / macOS / Linux 等）进行文档的编辑的与输出的软件工具
...
- **Hope 海洋之心**（当前主题）：[预览 ＞](https://madmaxchow.github.io/VLOOK/index.html)
...
![内置高颜值实用的主题](https://s1.ax1x.com/2020/05/08/Ynu94s.png "可自定义扩展更多样式")
...
![私人定制主题](https://s1.ax1x.com/2020/05/08/YnupNj.png "以上仅为私人定制的样品参考示例，非正式定制服务成品")
...
>
> [+] **这样做，也能为表格、mermaid 图表添加题注**
>
> > - 在表格前增加题注描述，使用语法是 VLOOK™ 扩展 GFM 的添加图片的 Markdown 语法；
...
`#注意#`如果你还需要对表格的列格式进行设置（如整列加粗、斜体等），VLOOK™ 也提供了同样好的支持，详见以下的[表格列格式设置](#表格列格式设置)
...
而且还是直接使用 Markdown 已有的语法，目前支持的格式包括：`**加粗**`、`*斜体*`、`<u>下划线</u>`、`==高亮==`、`~~删除线~~`。

> [+] **应用表格列格式的使用规范**
>
> > 只须在表头按以上标准的 Markdown 语法设置对应的格式即可实现整列格式化（暂不支持多格式的嵌套）

###### ![表格设置列格式后的混合排版效果]

| 正常   | **加粗列** | `#标签组|标签#`*斜体列* | <u>下划线</u> | ==高亮列==                | ~~删除线列~~              |
| ------ | -------- | -----------------: | :-----------: | ------------------------ | ------------------------ |
| 单元格 | 单元格   |             单元格 |    单元格     | 单元格`#标签#`单元格 | 单元格`#标签组|标签#`单元格 |
...
+ 语法：在行内代码内，以`#`作为起始内容，并以`#`结束，如：`` `#标签#` ``；

+ 还能可以通过在结束的`#`后添加指定不同的类型后缀标识，实现不同类型的标签会以不同的样式进行渲染：

   ![单标签示例]"（不指定后缀标识时，默认为 caution）"
...
- 语法：在「**单标签**」语法的基础上，以英文竖线符号`|`作为分隔符，如`` `#标签组|标签#` ``：
   ...
   ![双标签（嵌套标签）示例]"（不指定后缀标识时，默认为 caution）"
...
![图片默认样式（不指定在 Dark 模式时反色）](https://s1.ax1x.com/2020/04/30/JHXDDx.png?padding=true "透明的 PNG 图片会透出背景方格")

![指定切换至 Dark 模式后反色后会长这样（指定了在 Dark 模式时反色）](https://s1.ax1x.com/2020/04/30/JHXDDx.png?invert=in-dark&padding=true)
...
>  [+] **图片 URL 参数语法说明**
>
> > - **图片 URL 无参数时：**先在 URL 后添加英文问号`?`的再添加反色标记： `![图片题注](xxxxx.png?invert=in-dark)`
> > - **图片 URL 已有参数时：**多个参数间用英文特殊符号`&`分隔： `![图片题注](xxxxx.png?invert=in-dark&padding=true)`
...
VLOOK™ 主题会保持任务列表特性基础上，恢复列表编号的显示，并与上面[统一多级列表编号](#统一多级列表编号)保持一致。
...
从左到右分别为：**前一章节**、**当前章节**、**后一章节**，正如你现在看到顶部的导航栏那样。
...
在进行**文档评审或演示**时，可以激活逐段导航，对单段内容进行聚焦。
...
![Light / Dark 效果预览](https://s1.ax1x.com/2020/05/08/YueXpq.png "（← Light / → Dark）")
...
![](https://s1.ax1x.com/2020/04/14/GzVBHe.png)
...
- 下载并安装 [Typora](https://www.typora.io) ，启动后进入「偏好设置」，启用一些建议的选项，详细如下图：
  ![开启「Markdown 扩展语法」下的所有选项](https://s1.ax1x.com/2020/04/14/GzmKjP.png?invert=in-dark)
  
  
  
  ![开启「代码块」「公式」下的所有选项](https://s1.ax1x.com/2020/04/14/GzmQnf.png?invert=in-dark)
