...
|  演示辅助  | **聚光灯模式**    [>>>](#聚光灯模式)<br>太COOL了！在投影时可以让大家快速聚焦在关键内容上了（<kbd>S</kbd>激活，<kbd>⇧ Shift</kbd>调节大小） |
...
|  内容互动  | **代码块增强**    [>>>](#代码块增强)<br>在这你会发现连闷骚的代码都透着动感，高亮当行、复制～ |
...
![普通的图片示例](https://tva1.sinaimg.cn/large/00831rSTgy1gct75k6v6rj30u00u0b2b.jpg "（图片来源自互联网，版权尚不明确）")
...
- 对于文档中的长图、长表格、长代码会占用太多屏幕空间的情况，通过 VLOOK 可以实现自动折叠；
- 折叠后的内容可以一键展开：
  - 长图片（[查看示例](#这是普通的图片示例)）
  - 长图表（[查看示例](#mermaid－顺序图)）
  - 长表格（[查看示例](#特性清单)）
  - 长代码（[查看示例](#代码块增强)）

1. 展开/收起大纲、打印文档时都会自动进行重新的适配。
