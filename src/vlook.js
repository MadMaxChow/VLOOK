...
const _ = ``,
    ...
    _match_ = `match`,
    _matched_ = _match_ + `ed`,
    _data_keyword_match_ = _data__ + _keyword_ + `-` + _match_,
    ...
    __scheme_ = `-scheme`,
    _slide_ = `slide`,
    _data_slide_group_ = _data__ + _slide_ + `-` + _group_,
    ...
    _Warning_tt_ = `🚨 ${_Warning_} 🚨`,
    _Warning_tt_cn_ = `🚨 ${_Warning_cn_} 🚨`,
    ...
    _icoSearchDocLib_ = _ico_ + _Search_ + _DocLib_,
    _icoSearchHighlight_ = _ico_ + _Search_ + _Highlight_,
    ...
    _zen_ = `zen`,
    _Zen_ = `Zen`,
    _icoViewSlide_ = _ico_ + _View_ + _Slide_,
    _icoViewZen_ = _ico_ + _View_ + _Zen_,
    _status_ = `status`,
    _iframe_ = `iframe`,
    ...
    _Plugin_ = `Plugin`,
    ___v__ = `--` + _v__,
    _color__scheme_ = _color_ + __scheme_,
    ___v_color_scheme_ = ___v__ + _color__scheme_,
    _prefers_color_scheme_ = `prefers-` + _color__scheme_,
    ...
    ___thm_fig_ph_ =  ___thm__ + _fig_ + `-ph`,
    ___thm_fav_ = ___thm__ + `fav`,
    _vlook__ = _vlook_ + `-`,
    __autonum_ = `-autonum`,
    ...
    ___ac__ = `--ac-`,
    _var_ac_rd_ = V_ui_var(___ac__ + `rd`),
    _var_ac_rd_lg_ = V_ui_var(___ac__ + `rd` + __lg_),
    _var_ac_cy_ = V_ui_var(___ac__ + `cy`),
    _var_ac_cy_lg_ = V_ui_var(___ac__ + `cy` + __lg_),
    _var_ac_vn_ = V_ui_var(___ac__ + `vn`),
    _var_ac_vn_lg_ = V_ui_var(___ac__ + `vn` + __lg_),
    _var_ac_bk_ = V_ui_var(___ac__ + `bk`),
    _var_ac_bk_lg_ = V_ui_var(___ac__ + `bk` + __lg_),
    _var_ac_og_ = V_ui_var(___ac__ + `og`),
    _var_ac_og_lg_ = V_ui_var(___ac__ + `og` + __lg_),
    _var_db_ = V_ui_var(`--db`),
    _var_df_ = V_ui_var(`--df`),
    ...
    _v_btn__n__helper_ = `.${_v_btn_}.` + _helper_,
    _v_check_hash_ = _v__ + _check_ + __hash_,
    _v_color_scheme_ = _v__ + _color__scheme_,
    _v_content_ = _v__ + _content_,
    _v_content_helper_ = _v_content_ + `-` + _helper_,
    ...
    _v_shortcut_bar_ = _v__ + `shortcut-` + `bar`,
    _v_slide_hide_ = _v__ + _slide_ + `-` + _hide_,
    ...
    __opt_ = `-opt`,
    _v_font_style_opt_ = _v__ + _font_style_ + __opt_,
    ...
    __ident_ = `-ident`,
    _v_tbl_row_g_ident_folder_ = _v_tbl_row__ + `g${__ident_}-` + _folder_,
    ...
    _v_tbl_row_g_ident_ = _v_tbl_row__ + `g` + __ident_,
    _v_search_ = _v__ + _search_,
    _v_search_focus_ = _v_search_ + `-` + _focus_,
    _v_search_opt_ = _v_search_ + __opt_,
...
let gVer = `V2026.7`,
    ...
    // ----------
    // 插图占位图，因为 js 插件的执行会更早于主题文件的加载，所以这个为内置兜底的占位图，如能获取主题指定的需要在使用时进行替换
    gFigurePlaceholder = "data:image/svg+xml,%3Csvg width='108' height='108' viewBox='0 0 108 108' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Cdefs%3E%3Cpath d='M0 0h108v108H0V0z' id='a'/%3E%3C/defs%3E%3Cpath d='M108 0v108H0V0h108zM54.815 74.997a1.171 1.171 0 0 0-1.656.026l-4.565 4.71a1.562 1.562 0 0 0-.297 1.739l4.639 10.087a1.171 1.171 0 0 0 2.128 0l4.64-10.087a1.562 1.562 0 0 0-.298-1.74l-4.565-4.71zm-22.682-34.13C21.566 40.867 13 49.433 13 60c0 10.567 8.566 19.133 19.133 19.133 10.567 0 19.134-8.566 19.134-19.133 0-10.567-8.567-19.133-19.134-19.133zm43.734 0C65.3 40.867 56.733 49.433 56.733 60c0 10.567 8.567 19.133 19.134 19.133S95 70.567 95 60c0-10.567-8.566-19.133-19.133-19.133zm-43.734 5.857c7.333 0 13.277 5.944 13.277 13.276s-5.944 13.276-13.277 13.276c-7.332 0-13.276-5.944-13.276-13.276s5.944-13.276 13.276-13.276zm43.734 0c7.332 0 13.276 5.944 13.276 13.276s-5.944 13.276-13.276 13.276c-7.333 0-13.277-5.944-13.277-13.276s5.944-13.276 13.277-13.276zm18.893-24.94a2.929 2.929 0 0 0-3.854-1.519L54.005 36.31 17.104 20.265a2.929 2.929 0 1 0-2.336 5.372L52.815 42.18c.385.167.785.245 1.18.244h.02c.394.001.795-.077 1.18-.244l38.047-16.543a2.929 2.929 0 0 0 1.518-3.854z' fill='silver' xlink:href='%23a' fill-rule='evenodd' opacity='.25'/%3E%3C/svg%3E",
    // ----------
...
// 定义 data 的简化版
$.prototype.d = function (key, value) {
    return (value === gUndefined ? this.data(key) : this.data(key, value));
}
...
// 定义 clone 简化版
$.prototype.cl = function (value) {
    return (value === gUndefined ? this.clone() : this.clone(value));
}

// 定义 click 简化版
$.prototype.ck = function (callback) {
    // 避免事件被重复绑定，导致事件处理函数被多次调用
    if (V_devMode) {
        this.d(_bound_ + _click_)
            && ERROR(`re` + _bound_ +___+ _click_ +___+ this.po(_outerHTML_));
        this.d(_bound_ + _click_, gTrue);
    }
...
// 定义 cloneNode 简化版
function JS_cloneNode(source, value) {
    return source.cloneNode(value);
}
 ...
 * 将调试信息输出到页面上
 */
let DebugPanel_ui = gUndefined;

// 初始化调试信息栏
function Debug_init() {
    DebugPanel_ui = V_byID(_v__ + _debug_);
    if (!DebugPanel_ui)
        return;
    ...
    DebugPanel_ui.ck(() => {
        ...
        JQ_hasClass(DebugPanel_ui, _mini_)
            ? Debug_open()
            : Debug_mini();
    });
    ...
    V_on(gWindow, _error_, e => {
        Debug_append(_error_, [
            `Uncaught`,
            e.message,
            e.filename + `:` + e.lineno + `:` + e.colno
        ]);
    });
    V_on(gWindow, `unhandledrejection`, e => {
        Debug_append(_error_, [
...
// 打开调试信息栏
function Debug_open() {
    JQ_removeClass(DebugPanel_ui, _mini_);
    JQ_addClass(DebugPanel_ui, _opened_);
}

// 最小化调试信息栏
function Debug_mini() {
    JQ_removeClass(DebugPanel_ui, _opened_);
    JQ_addClass(DebugPanel_ui, _mini_);
}

// 添加调试信息
function Debug_append(type, args) {
    DebugPanel_ui = V_byID(_v__ + _debug_);
    if (!DebugPanel_ui || V_isLength0(args.join(___)))
        return;
    ...
    JQ_addClass(DebugPanel_ui, type);
    JQ_addClass(line, type);
    line.hm(V_ui_strong(lineNum) + `: ` + args.join(___));

    // V_devMode = gFalse;
    if (!JQ_hasClass(DebugPanel_ui, _opened_)) {
        if (V_devMode)
    ...
    if (warnNum + errorNum > 0)
        DebugPanel_ui.a(_data_num_, warnNum + errorNum);
    else
        DebugPanel_ui.a(_data_num_, `i`);

    // 控制行数不超过 100 行
    V_length(DebugPanel_ui.ch()) > 100
        && DebugPanel_ui.ch().last().rm();
    DebugPanel_ui.pp(line);
}
 ...
 */
function V_ui_setScrollTop(pos, scrollMarginTopSource = gUndefined, scrollContainer = gUndefined) {
    // 标记为程序触发的滚动
    ...
    // 重置更多内容遮罩的高度
    V_setVarVal(___v_more_mask_ + `-a-h`, 0);
}

/**
 * 滚动指定对象到可视区域
 * @param target 目标对象
 * @param inlineValue inline 参数值，指水平方向的滚动方式
 * @param blockValue block 参数值，指垂直方向的滚动方式
 */
function V_ui_setScrollIntoView(target, inlineValue, blockValue) {
...
function V_parseMarkdown_link(text) {
    let result = text.m(/^(\[(?<text>(?:\\.|[^\[\]])*)]\(\s*(?<url><[^>]+>|(?:(?:\\[()])|[^\s()\\]+|\((?:\\[()]|[^()\\])*\))+)((?:\s+(?:"(?<title1>[^"]*)"|'(?<title2>[^']*)'|\((?<title3>[^)]*)\)))?\s*)\))$/);
    if (result === gNull)
        return gNull;
    ...
    return {
        ...
        text : result[2].x(),
        title : (V_length(result) > 5 && result[5] !== gUndefined) ? result[5].x() : _
    }
}

/**
 * 折叠 TOC 指定层级的章节
 * @param level 指定层级：1-5
 */
function V_collapseToc(level) {
    level >= 1 && level <= 5
        && gVlookToc.ch(`.${_md_toc__}h` + level + V_attrCSS(_data_node_, 1) + V_attrCSS(_data_folded_, _false_)).e((index, element) => {
            iNavCenter.toc.toggleFolder($(element).a(_id_), _close_, gTrue);
        });
}
...
function V_ui_copyrightInfo(showIcon = gTrue, showVersion = gTrue) {
    return V_ui_div(_, _v_copyright_ +___+ _v_transition_all_,
        (showIcon
            ? V_ui_svgIcon(_icoVLOOK_, 20, 20, _current_color_, _v_copyright_svg_ico_) + _2nbsp_
            : _
        )
        + V_ui_a(_, _httpsPrefix_ + _githubVlook_, _Powered_by_ + V_ui_strong(_VLOOK_ + `™`), __blank_)
        + (showVersion
            ? ` (${gVer})`
            : _
        )
...
function V_ui_svgIcon(icon, w, h, color, classValue, extAttr) {
    ...
    return `<${_svg_} ${_width_}="${w}px" ${_height_}="${h}px" `
        + _class_ + `="${_v_svg_i_}-` + _small_ + classValue + `"`
        + extAttr + `>`
            + `<${_use_} ${_class_}="${_v_svg_i_}-${color}" ` + V_attr(_xlink_href_, "#" + icon) + `>`
            + `</${_use_}></${_svg_}>`;
}
 ...
 * @param str 源内容
 */
function V_ui_escapeHTML(str) {
    let div = V_createElement(_div_);//document.createElement(_div_);
    div.textContent = str;
    return JS_innerHTML(div);
}
...
function V_ui_genGradColorCSS(gradientColors, fade) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        css += V_ui_var(___ac__ + color + fade);
        if (i < count - 1)
            css += `,`;
    }
    return css;
...
function V_ui_initI18n() {
    iToolbar.btns[_nav_center_].a(_data_tips_, V_lang_text6() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`O`)));

    iToolbar.btns[_zen_].a(_data_tips_, V_lang_text39() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`Z`)));
    iToolbar.btns[_slide_].a(_data_tips_, V_lang_text103() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`S`)));

    iToolbar.btns[_paragraph_nav_].a(_data_tips_, V_lang_text8() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`J`) + ` , ` + V_ui_wrap_kbd(`K`))
    ...
    iToolbar.btns[_spotlight_].a(_data_tips_, V_lang_text9() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`L`))
        + V_ui_sup(_, _, _2br_ + V_lang_text(33, [
            `切换聚光灯大小`,
            `Toggle the size of the spotlight`
        ]) + _br_ + V_ui_wrap_kbd(`⇧ ` + _Shift_)));

    iToolbar.btns[_laser_pointer_].a(_data_tips_, V_lang_text10() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`P`)));
...
function V_initHotkey() {
    ...
    jqDocument.on(_keydown_, event => {
        ...
        // 过滤在输入框、文本区域或具有 contenteditable 属性的元素中触发的按键事件
        // 避免干扰正常的文本输入和编辑操作
        if ((_input_ + _text_ + __area_).i(V_tagName($(V_eventTarget(event)))) > -1 ||
            V_eventTarget(event).isContentEditable)
            return;
        ...
        if (iSlideView.hotkey(key, combKeys, event)) return;
            ...
            switch (key) {
                ...
                case `/`: // / 导航中心搜索
                    ...
                    navigator.clipboard.readText().then(value => {
                            iNavCenter.kw.processInput(_);
                        }).catch(value => {
                            ERROR(`Get Clipboard ${_Failed_}：`, value);
                        });
                    break;
                case `a`: // D Light / Dark Mode
                case `A`: // D Light / Dark Mode
                ...
                case `k`: // J,K 段落漫游
                case `K`:
                    iToolbar.btns[_paragraph_nav_].tr(_click_);
                    break;
                case `p`: // P 激光笔
                case `P`:
                    iToolbar.btns[_laser_pointer_].tr(_click_);
                    break;
                case `o`: // O 聚光灯
                case `O`:
                    iToolbar.btns[_spotlight_].tr(_click_);
                    break;
                case `e`: // E 异常链接 / 地址
                case `E`:
                ...
                case `H`:
                    ...
                    break;
                case `s`: // S 幻灯片视图（隐藏插件界面）
                case `S`:
                    iToolbar.btns[_slide_].tr(_click_);
                    break;
                case `z`: // Z 宁静视图（隐藏插件界面）
                case `Z`:
                    iToolbar.btns[_zen_].tr(_click_);
                    break;
            }
        }
    });
 ...
 */
function V_getUrlQueryParams(url) {
    if (url === gUndefined)
        return {};

    let h = url.i(`#`);
    url = h > -1 ? url.ss(0, h) : url; // 只截取 URL 中 # 前的内容

    let si = url.i(`?`),
        queryStr = url.ss(si > -1 ? si + 1 : V_length(url), V_length(url)), // 获取url中`?`符后的字串
        paramSet = {}, // 保存参数数据的对象
 ...
 * @param url 完整的 URL 内容
 */
function V_getUrlHash(url) {
    if (url === gUndefined)
        return _;

    let i = url.i(`#`);
    return i > -1 ? url.ss(i, V_length(url)) : _;
}
 ...
 * @param url 完整的 URL 内容
 */
function V_getUrlQueryString(url) {
    if (url === gUndefined)
        return _;

    let i = url.i(`?`);
    return i > -1 ? url.ss(i + 1, V_length(url)) : _;
}
 ...
 * @param url 完整的 URL 内容
 */
function V_getUrlWithoutHash(url) {
    if (url === gUndefined)
        return _;

    let i = url.i(`#`);
    return i > -1 ? url.ss(0, i) : url;
}
 ...
 * @param url 完整的 URL 内容
 */
function V_getUrlWithoutQueryAndHash(url) {
    if (url === gUndefined)
        return _;

    let queryIdx = url.i(`?`);
    if (queryIdx > -1)
        url = url.ss(0, queryIdx);
    else {
        let hashIdx = url.i(`#`);
        if (hashIdx > -1)
            url = url.ss(0, hashIdx);
    }
    return url;
}
 ...
 * @param url 完整的 URL 内容
 */
function V_getUrlFilenameWithoutQueryAndHash(url) {
    if (url === gUndefined || url.sW(_data_))
        return _;
    return V_getUrlWithoutQueryAndHash(url.ss(url.li(`/`) + 1));
}
 ...
 * @param url URL 地址
 */
function V_getPath(url) {
    let qi = url.i(`?`),
        ti = url.ss(0, qi).li(`/`),
        pi = ti < 0 ? 0 : ti;
    return url.ss(0, pi + 1);
}
            ...
            SVGInject(img[0], {
                ...
                afterInject : function (img, svg) {
                    ...
                    !V_isNotFigure(hash, container)
                        && ContentHelper_bind(svgObj, _fig_ + _suffixSvg_);
                    // 对颜色进行替换的适配处理
...
/**
 * 获取 HTML 文档使用的模板主题兼容代码
 */
function V_getTemplateThemeCompCode() {
    return V_getVarVal(___v_theme__ + _compcode_);
}
...
/**
 * 获取 JavaScript 原生事件对象的 target 属性值
 * @param event 事件对象
 */
function V_eventTarget(event) {
    return event.target;
}

/**
 * 获取 JavaScript 原生事件对象的 origin 属性值
 * @param event 事件对象
 */
function V_eventOrigin(event) {
    return event.origin;
}

/**
 * 获取 JavaScript 原生事件对象的 originalEvent 属性值
 * @param event 事件对象
 */
function V_eventOriginalEvent(event) {
 ...
 * @param target 对象
 */
function V_textNodeContent(target) {
...
/**
 * 获取 JavaScript 原生对象的 innerHTML 内容
 * @param jsElement 对象
 */
function JS_innerHTML(jsElement) {
    return jsElement.innerHTML;
}
...
function V_getSrcValue(target) {
    ...
    return (src === gUndefined || src.sW(_data_) ? target.a(_data_src_) : src);
}
...
function V_init() {
    ...
    // 幻灯片视图
    iSlideView = new SlideView();
    V_isLength0(iSlideView)
        && ALERT(_Failed__ + `i${_Slide_} ]`);
    sw.ed(`    ├ ` + _Slide_ + `: `);
    ...
    // 导航中心
    sw.st();
    gScrollMarginTop = JS_parseInt(V_getVarVal(___v__ + _scroll_ + `-` + _margin_top_)) * 2;
    ...
    if (V_isLength0(iToolbar))
        ...
        iToolbar.add(_nav_center_, () => {
            ...
            // 针对当前只显示了文库的处理
            NavCenter_filterText() === _doclib_
                && NavCenter_clearFilter();
        ...
        // 幻灯片模式视图
        iToolbar.add(_slide_, () => {
            // 与 宁静视图 互斥
            View_current() === _normal_
                && iSlideView.enable();
        });
...
function V_initKernel() {
    ...
    gWindow.onmessage = event => {
        ...
        // ！！！不要删除！！！用于测试效果
        // 用于模拟测试使用
        // msg = `chk-update:no,latest:2026.8`;
        // msg = `chk-update:yes,latest:27.1`;
        // msg = `chk-update:yes`;
        // msg = `chk-update:no`;
        LOG(`<- ${V_eventOrigin(event)}\n` + msg);
        if ((V_eventOrigin(event) !== NewVersion_orgPagesDev
            || V_eventOrigin(event) !== NewVersion_orgGitHub)
            && (!msg.sW(_chk_update_) || msgLen > 50)) {
                INFO(_Invalid_ + ` ${_data_} from ` + V_eventOrigin(event));
                INFO(`[${msgLen}]`, msg);
            return;
        }
        ...
        // ！！！不要删除！！！用于测试效果
        // 用于模拟测试使用
        // msg = `chk-update:yes,latest:2026.4`;
        // msg = `chk-update:no,latest:26.2`;
        // msg = `chk-update:yes`;
        // 处理新版本检查结果
    ...
    jq_Window.on(_hash_ + _change_, () => {
        ...
        if (V_length(hashObj) > 0) {
            ...
            !V_ui_switchToTabItem(hashObj)
                // 非页签组相关内容的处理（CSS 中的 scroll-margin-top 需要配合滚动处理才能生效）
                && V_ui_setScrollTop(hashObj.oT(), hashObj, gUndefined);
        }
...
// 获取语言 ID 103 内容
function V_lang_text103() {
    return V_lang_text(103, [
        `幻灯片视图`,
        _Slide_ +___ + _View_
    ]);
}
// 获取语言 ID 104 内容
function V_lang_text104() {
    return V_lang_text(104, [
        `筛选`,
        `Filter`
    ]);
}
// 获取语言 ID 105 内容
function V_lang_text105() {
    return V_lang_text(105, [
        `高亮`,
        _Highlight_
    ]);
}
...
function Fullscreen_init() {
    ...
    V_ui_bindTouchStyle(Fullscreen_btn_close, V_lang_text17() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(_ESC_)));
    Fullscreen_btn_close.ck(() => {
        Fullscreen_hide();
    });
    ...
    Fullscreen_btn_picInPic.ck(() => {
        Fullscreen_hide();
        ContentHelper_lastHover = Fullscreen_target;
        PicInPic_show();
    });
...
function Fullscreen_show(type, target) {
    ...
    if (V_isLength0(target))
        target = ContentHelper_lastHover;
...
function Fullscreen_bindEvent() {
    ...
    Fullscreen_container.on(_pointer_ + _down_, event => {
        ...
        if (!targetOrg || (V_eventTarget(event) !== targetOrg && !$.contains(targetOrg, V_eventTarget(event))))
            return;
    ...
    Fullscreen_container.on(_touchstart_, event => {
        let touches = V_eventOriginalEvent(event) ? V_eventOriginalEvent(event).touches : event.touches;
        // 如果是双指，初始化距离
...
let ContentHelper_ui = gUndefined,
    ContentHelper_btns_copyContent = gUndefined, // 复制内容
    ContentHelper_btns_fullscreen = gUndefined, // 表格全屏显示
    ContentHelper_btns_tableFreeze = gUndefined, // 表格冻结列
    ContentHelper_btns_tableCross = gUndefined, // 表格阅读模式（十字光标）
    ContentHelper_btns_wrap = gUndefined, // 表格、代码块换行、不换行
    ContentHelper_btns_picInPic = gUndefined, // 「画中画」
    // 最后显示新标签打开按钮的内容（插图/表格等
    ContentHelper_lastTarget = gUndefined, // 用于代码、标签
    ContentHelper_lastHover = gUndefined,
    ContentHelper_lastContentType = gUndefined;

function ContentHelper_init() {
    ContentHelper_ui = V_byClass(_v_content_helper_);

    ContentHelper_btns_fullscreen = $(_v_btn__n__helper_ + `.` + _fullscreen_); // 全屏显示
    ContentHelper_btns_copyContent = $(_v_btn__n__helper_ + `.` + _copy_); // 复制内容
    ContentHelper_btns_tableFreeze = $(_v_btn__n__helper_ + `.` + _table_freeze_); // 表格冻结列
    ContentHelper_btns_tableCross = $(_v_btn__n__helper_ + `.` + _table_cross_); // 表格阅读模式（十字光标）
    ContentHelper_btns_wrap = $(_v_btn__n__helper_ + `.` + _wrap_); // 表格、代码块换行、不换行
    ContentHelper_btns_picInPic = $(_v_btn__n__helper_ + `.` + _pic_in_pic_); // 「画中画」

    ToolTips_bind(ContentHelper_btns_fullscreen, V_lang_text(24, [
        `全屏`,
        `Full Screen`
    ]));

    ToolTips_bind(ContentHelper_btns_tableFreeze, V_lang_text(29, [
        `冻结列`,
        `Freeze Column`
    ]));

    ToolTips_bind(ContentHelper_btns_tableCross, V_lang_text(25, [
        `阅读` + _Mode_cn_,
        _Reading_ +___+ _Mode_
    ]));

    ToolTips_bind(ContentHelper_btns_wrap,  V_lang_text(26, [
        `换行 / 不换行`,
        `Line ${_break_} / No ` + _line_ +___+ _break_
    ]));

    ToolTips_bind(ContentHelper_btns_picInPic, V_lang_text27());

    ContentHelper_ui.on(_mouseleave_, event => {
        !v_ui_mouseDropIn(ContentHelper_lastHover, event)
            && ContentHelper_hide();
    });

    // 开/关：全屏显示
    ContentHelper_btns_fullscreen.ck(() => {
        ContentHelper_hide();
        Fullscreen_show(ContentHelper_lastContentType);
    });

    // 开/关：表格冻结列
    ContentHelper_btns_tableFreeze.ck(() => {
        ContentHelper_lastHover.tc(ExtTable_freezeClass);
    });

    // 开/关：表格阅读模式（十字光标）
    ContentHelper_btns_tableCross.ck(event => {
        TableCross_toggle(event);
    });

    // 开/关：表格、代码块的换行、不换行
    ContentHelper_btns_wrap.ck(() => {
        __actionWrap();
    });

    // 复制
    ToolTips_bind(ContentHelper_btns_copyContent);
    ContentHelper_btns_copyContent.ck(event => {
        __actionCopy(V_eventCurrentTarget(event), event);
    });

    // 画中画
    ContentHelper_btns_picInPic.ck(() => {
        PicInPic_show();
    });

    // 处理【换行、不换行】
    function __actionWrap() {
        if (ContentHelper_lastHover === gUndefined)
            return;

        // 表格
        if (ContentHelper_lastContentType === _table_)
            TableWrap_toggle();
        // 代码块
        else if (ContentHelper_lastContentType === _codeblock_)
    ...
    function __actionCopy(target, event) {
        ...
        // 代码块
        if (ContentHelper_lastContentType === _codeblock_)
            ExtCodeBlock_copyContent(target, event);
        // 图片插图
        else if (ContentHelper_lastContentType === _fig_ + _suffixImg_)
 ...
 */
function ContentHelper_bind(target, contentType) {
    // 鼠标悬停时事件处理
    target.on(_mouseenter_, () => {
        if (gIsTouching || ContentHelper_btns_copyContent === gUndefined)
            return;
        __showHelper();
    ...
    function __showHelper() {
        V_ui_removeAnimate(ContentHelper_ui);

        ContentHelper_lastHover = target;
        ContentHelper_lastContentType = contentType;

        if (contentType === _codeblock_)
            ContentHelper_btns_copyContent.a(_data_tips_, V_lang_text(30, [
                `复制代码`,
                _Copy_ +___+ _code_
            ]));
        else if (contentType === _fig_ + _suffixImg_)
            ContentHelper_btns_copyContent.a(_data_tips_, V_lang_text(31, [
                `复制图片地址`,
 ...
 * 显示指定内容的内容助手
 */
function ContentHelper_show() {
    if (ContentHelper_lastHover === gUndefined)
        return;

    // 重置各按钮状态
    V_unEnable(ContentHelper_btns_copyContent);
    V_unEnable(ContentHelper_btns_fullscreen);
    V_unEnable(ContentHelper_btns_tableFreeze);
    V_unEnable(ContentHelper_btns_tableCross);
    V_unEnable(ContentHelper_btns_wrap);
    V_unEnable(ContentHelper_btns_picInPic);

    // 插图
    if (ContentHelper_lastContentType.sW(_fig_)) {
        // 全屏
        V_enable(ContentHelper_btns_fullscreen);
        // 图片插图：非 svg
        !ContentHelper_lastContentType.eW(_svg_)
            && V_enable(ContentHelper_btns_copyContent);
        // 画中画
        V_enable(ContentHelper_btns_picInPic);
    }
    // 表格
    else if (ContentHelper_lastContentType === _table_) {
        // 全屏
        V_enable(ContentHelper_btns_fullscreen);
        // 冻结列
        if (!V_isLength0(ExtTable_freezeClass)) {
            V_enable(ContentHelper_btns_tableFreeze);
            let c = ContentHelper_lastHover.a(_class_);
            c !== gUndefined && c.i(ExtTable_freezeClass) > -1
                ? JQ_addClass(ContentHelper_btns_tableFreeze, _selected_)
                : JQ_removeClass(ContentHelper_btns_tableFreeze, _selected_);
        }
        // 阅读模式
        V_enable(ContentHelper_btns_tableCross);
        ContentHelper_lastHover.a(_data_tbl_x_) === _true_
            ? JQ_addClass(ContentHelper_btns_tableCross, _selected_)
            : JQ_removeClass(ContentHelper_btns_tableCross, _selected_);
        // 换行、不换行
        V_enable(ContentHelper_btns_wrap);
        TableWrap_isWrap(ContentHelper_lastHover)
            ? JQ_addClass(ContentHelper_btns_wrap, _selected_)
            : JQ_removeClass(ContentHelper_btns_wrap, _selected_);
        // 画中画
        V_enable(ContentHelper_btns_picInPic);
    }
    // 代码块
    ...
    // 公式
    else if (ContentHelper_lastContentType === _formula_)
        // 画中画
        V_enable(ContentHelper_btns_picInPic);

    // ----------------------------------------
    // 计算助手显示的位置
    let caption = ContentHelper_lastHover.p(),
        container = caption.p(),
        left, top, pos;

    left = ContentHelper_lastHover.oW() < container.oW()
        ? ContentHelper_lastHover.oL() + ContentHelper_lastHover.oW()
        : container.oL() + container.oW();

    pos = ContentHelper_lastHover.oT() - V_ui_getScrollTop() - 70; // 70 是工具栏高度 + 间距
    top = pos > 0
        ? (ContentHelper_lastHover.oT() + 3)
        : (ContentHelper_lastHover.oT() - pos);
    // 显示
    ContentHelper_ui.c(_left_, parseInt(left - ContentHelper_ui.w() - 6))
        .c(_top_, top);

    // 须延时后再执行显示，让以上代码先完成
    V_later(() => {
        V_ui_addAnimate(ContentHelper_ui);
        V_ui_fadeShow(ContentHelper_ui);
    }, 100);
}
 ...
 * 隐藏内容辅助动作按钮
 */
function ContentHelper_hide() {
    V_ui_fadeHide(ContentHelper_ui);
}
...
function PicInPic_show() {
    let source = ContentHelper_lastHover,
    ...
    // 图片的处理，待完成显示和加载克隆对象后，根据内容调整画中画的展示
    if (tagName === _img_)
        pic.on(_load_, () => {
            PicInPic_calcSize();
    ...
    function __cloneForPicInPic (source) {
        let tagName = V_tagName(source),
            openedAll = (tagName === _table_) ? RowGroup_openAll(source, _auto_) : gFalse;

        // 将来源对象的内容进行克隆
        let newClone = source.cl();
...
function PicInPic_fitContentSize(source) {
    ...
    PicInPic_ui_body.c(_width_, w > 0 && wWithPadding < PicInPic_size_width
        ? wWithPadding
        : PicInPic_size_width);
    ...
    if (h > 0 && hWithPadding < PicInPic_size_height) {
        // 针对会进行等比缩放的对象进行高度微调
        if ((tagName === _img_ || tagName === _svg_)
            && h > source.h()) {
                h = source.h();
        ...
        PicInPic_ui_body.c(_height_, PicInPic_size_height);
...
function Spotlight(radius) {
    ...
    T.show = () => {
        ...
        iInfoTips.bubble(V_ui_strong(V_lang_text9()) + V_ui_sup(_, _, _2br_ + V_ui_wrap_kbd(`⇧ ` + _Shift_) +___+ V_lang_text(33, [
                `切换聚光灯大小`,
                `Toggle the size of the spotlight`
            ]))
            + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(_ESC_) +___+ V_lang_text17())
            , 2000, gUndefined, `ye`);
...
let ShortcutBar_ui = gUndefined,
    iNavHistory = gUndefined,
    NH_ViewHistory_ui = gUndefined;

function ShortcutBar_init() {
    ShortcutBar_ui = V_byClass(_v_shortcut_bar_);
    V_ui_addAnimate(ShortcutBar_ui);
    V_ui_addAnimate(ShortcutBar_ui.ch());

    NH_ViewHistory_ui = V_byClass(_v_view_history_);
    ToolTips_bind(NH_ViewHistory_ui, V_lang_text65() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`H`)));

    iNavHistory = new NavHistory(this, gTrue);

    NH_ViewHistory_ui.ck(() => {
 ...
 * 启用导航历史入口
 */
function ShortcutBar_enableHistory() {
    V_enable(NH_ViewHistory_ui);
    !isNaN(ResumeReading_lastPosition)
        && iNavHistory.add(ResumeReading_lastPosition.toString());
}
...
function ResumeReading_init() {
    ...
    // 点击跳转
    ResumeReading_ui.ck(() => {
        V_ui_setScrollTop(ResumeReading_lastPosition, gUndefined, gUndefined);

        V_unEnable(ResumeReading_ui);
        ShortcutBar_enableHistory();
...
function ResumeReading_dispose(restore) {
    ...
    // 不是恢复显示时才处理
    if (!restore && !isNaN(ResumeReading_lastPosition)) {
        V_later(() => {
            V_unEnable(ResumeReading_ui);
            ShortcutBar_enableHistory();
        }, 6000);
...
function NavCenter(mask, runMode = _auto_) {
    ...
    T.kw = new SearchBar(T.__keywordBody, _index_, gTrue);
    ...
    T.show = () => {
        ...
        if (T.lastType === _block_) {
            ...
            // 如：std, flat, edge
            JQ_addClass(T.toolbar.ui, gNavCenterStyle);

            T.ui.c(_top_, V_ui_hasCover() && !V_ui_inHeader() ? 20 : 70);

            // 更新工具栏导航中心按钮图标
            !V_isMobile()
                && T.toolbar.sel(_nav_center_);
    ...
    T.hide = (runMode = _auto_) => {
        ...
        // 如：std, flat, edge
        JQ_removeClass(T.toolbar.ui, gNavCenterStyle);
...
function NavCenter_init() {
    ...
    // 不指定打开，则默认收起折叠所有含子章节的 h1 章节
    let tocAutoCloseLevel = V_getQueryParamVal(_toc_);
    V_collapseToc((tocAutoCloseLevel !== gNull) ? JS_parseInt(tocAutoCloseLevel) : 1);

    return gTrue;
}

// 返回导航中心关键字搜索的输入内容
function NavCenter_filterText() {
    return iNavCenter.kw.in.v();
}

// 返回导航中心关键字搜索的输入内容
function NavCenter_clearFilter() {
    return iNavCenter.kw.clear();
}
...
function ChapterNav() {
     ...
     */
    T.hotkey = (key, combKeys, event) => {
        if (V_doc_block || View_current() !== _normal_)
            return;
...
function ParagraphNav_init() {
    ...
    // 初始化
    $(_h1__6_ + `,ul>li,${_details_},ol>li,${_idWrite_}>p` + V_attrCSS(_class_, _md_toc__ + _content_, `!`) + `,blockquote>p,figure,.${_md_alert_}>p,.${_md_diagram_panel_},.MathJax_SVG_Display`).e((index, element) => {
        let item = $(element);
...
let ColorScheme_auto = gTrue, // 是否为 auto 模式
    ...
    ColorScheme_vars = []; // 内置的与颜色方案相关的 CSS 变量集

/**
 * 初始化
 */
function ColorScheme_init() {
    let scheme = V_data_read(_color__scheme_, gTrue);
    if (scheme !== gNull && (scheme === _light_ || scheme === _dark_))
        ColorScheme_auto = gFalse;

    // 获取主题中与颜色方案相关的 CSS 变量集
    ColorScheme_vars = V_getVarVal(`--thm-cs-vars`).sp(`,`);
...
function ColorScheme_refresh(force) {
    if (!force && ColorScheme_auto) {
        V_data_remove(_color__scheme_, gTrue);
        ColorScheme_scheme = ColorScheme_systemScheme;
    }
    else if (!force)
        V_data_write(_color__scheme_, ColorScheme_scheme, gTrue);

    // 修改主题颜色外观相关变量
    V_changeCssVarSet(ColorScheme_vars, (ColorScheme_scheme === _dark_) ? `dk` : `lg`);
...
function FontStyle_monitorLoading() {
    ...
    loadingFonts.forEach(font => {
        font.loaded.then(() => {
            ...
            INFO(`⚠️⚠️ ${font.status}: ${font.family} ${font.style} ${font.weight} / ${font.unicodeRange}`);
        });
    });
...
function FontStyle(mask, styleName) {
    ...
    T.loadFromCSS = (cssUrl, linkId) => {
        return new Promise((resolve, reject) => {
            ...
            link.onerror = () => resolve(_Failed_ + ` to ${_load_} CSS "${cssUrl}"`);
    ...
    T.initWF = () => {
        ...
        V_getMeta(_vlook__ + `fs`).e((index, element) => {
            ...
            linkId = linkId.replace(/\./g, "-") + `-` + T.style;

            // 实时加载指定字体风格的字体样式定义文件
            T.loadFromCSS(content + (V_devMode ? `dev` : gVer) + `/fs-${T.style}-min.css`, linkId)
                .then(gConsole.log)
                .catch(gConsole.error);
        });
    }
...
function Footnote_init() {
    ...
    V_length(vkFooterArea) > 0 && iNavCenter.lib !== gUndefined
        && iNavCenter.lib.addHashItem(V_ui_svgIcon(_icoDocLibRef_ + gIconStyle_uico, 20, 18, (gNavCenterStyle === _edge_ ? _btnFc_ +___ : _) + _current_color_), V_lang_text22(), _vk_footer_area_);
    ...
    a.ck(event => {
        ...
        let _t = V_eventCurrentTarget(event),
            ...
            target = $(`a` + V_attrCSS(_name_, `df` + nameValue)
                + `,a` + V_attrCSS(_id_, `df` + nameValue)).p().cl();
...
function StatusBar_init() {
    ...
    V_ui_bindTouchStyle(StsLinkMap_ui, V_lang_text3() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`M`)));
    StsLinkMap_ui.ck(() => {
    ...
    V_ui_bindTouchStyle(StsPrint_ui, V_lang_text32() + _2br_
        + V_ui_sup(_, _, V_ui_getCmdCtrlKey(3, gTrue) +___+ V_ui_wrap_kbd(`P`)));
    StsPrint_ui.ck(() => {
    ...
    V_ui_bindTouchStyle(StsFontStyle_ui, V_lang_text41());
    StsFontStyle_ui.ck(() => {
        iFontStyle.tg();
    });
    ...
    gThmColorScheme = V_getVarVal(___thm__ + _color__scheme_);
    if (V_pageMode !== _max_) {
        StsColorScheme_ui.hide();
        StsFontStyle_ui.hide();
        StsPrint_ui.hide();
    }
    if (gThmColorScheme !== _auto_)
        StsColorScheme_ui.hide();
...
function StatusBar_onFocus(source) {
    ...
    // 显示聚焦提示信息
    StsFocusInfo_ui.hm(
        V_ui_label(_, _, ui.hm() + source.desc + _2nbsp_ + `•` + _2nbsp_  + V_ui_wrap_kbd(_ESC_) +___+ V_lang_text17())
    );
...
function StsColorScheme_init() {
    V_ui_bindTouchStyle(StsColorScheme_ui, V_lang_text45() + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`A`)));

    StsColorScheme_updateIcons();

    StsColorScheme_ui.ck(() => {
        if (gThmColorScheme !== _auto_)
            return;
...
function StsColorScheme_updateIcons(showBubble) {
    ...
    // 清除微调样式
    JQ_removeClass(StsColorScheme_ui, _light_ +___+ _dark_);
    ...
    if (!ColorScheme_auto) {
        ...
        JQ_addClass(StsColorScheme_ui, _selected_ +___+ ColorScheme_scheme);
    }
    // 针对 auto 模式
    else {
    ...
    // 更新提示
    ToolTips_bind(StsColorScheme_ui, info + V_ui_strong(mode) + _2br_ + V_ui_sup(_, _, V_ui_wrap_kbd(`A`)));
    // 工具栏提示没有显示时，则显示泡泡信息
...
function NewVersion_yes(latestVer) {
    StsNewVersion_ui.hm(V_ui_svgIcon(_icoNewVersion_ + gIconStyle_uico, 20, 20, _text_) + `V` + latestVer);
...
function LinkTool_checkLink() {
    ...
    $(_idWrite_ + ` :is(` + _h1__6_ + V_not(__first_child_) + `),` // h1-h6
        ...
        // 初始化「章节」的交叉引用源数据
        V_length(text) > 0 && headerNum !== gUndefined
            && xRefSet.pu([_t.a(_id_), headerNum.x()]);
    });
    ...
    $(_idWrite_ + ` a, .${_md_toc_item_}>a`).e((index, element) => {
        ...
        // --------------------
        // 检索是否存在与该内链对应的锚点，用于显示交叉引用的前缀信息
        if (href.sW(`#`) && !href.sW(_mjx__) && V_length(_t.ch(_span_)) > 0) {
            let anchor = href.ss(1, V_length(href));
            // 没有检索到对应的锚点（只有 # 也视为正常）
            !V_isLength0(anchor) && LinkTool_notFoundHash(anchor)
...
function BgMask(id, align, close) {
    ...
    // 绑定 VLOOK logo 的点击事件
    T.ui.f(`.${_v_copyright_} .` + _v_copyright_svg_ico_).ck(event => {
        env.show(V_eventCurrentTarget(event));
    });

    // 点击遮罩隐藏联动对象
    T.ui.ck(event => {
        if (V_tagName($(V_eventTarget(event))) === `a`)
            return;
...
function InfoTips(mask) {
     ...
     * @param extcss 扩展样式名称
     */
    T.bubble = (message, delay, pos, extcss) => {
        T.show(message, delay, gFalse, _bubble_, pos, extcss);
    }
     ...
     * @param extcss 扩展样式名称
     */
    T.show = (message, delay, mask, type, targetPos, extcss) => {
        T.aniTimer = V_clearTimer(T.aniTimer);

        T.ui.hm(message);

        // 先重置为默认值
        JQ_removeClass(T.ui, `lm ye og rd`);
...
function Caption_groupPreppare() {
    ...
    $(_idWrite_ + `>:is(${_h1__5_})`).e((index, hx) => {
        let header = $(hx),
            betweenSet = header.nU(_h1__5_);
 ...
 * @returns 题注容器对象
 */
function Caption_wrapContainer(target, type) {
    let container = (type.sW(`c`) ? target : target.p()),
        containerTag = V_tagName(container);

    // 如果容器不是 figure(指img、table) / div(指mermaid)，则添加一个 figure 作为容器
    if (containerTag !== _figure_ && containerTag !== _div_) {
        target.wrap(V_ui_figure(_, _, _));
        container = target.p();
    }

    // 设置容器数据、样式
    if (container.a(_id_) !== _write_) {
        container.a(_data_cntr_, type);
...
/**
 * 针对非插图、多媒体类（内嵌tag）生成题注
 * @param target 需要添加题注的对象
 * @param type 题注类型名称
 */
function Caption_genForText(target, type) {
    let captionPrefix = Caption_prefix(target, V_lang_text56(), V_doc_counter_table),
        caption,
        containter = target,//(type.sW(`c`)) ? target.p() : target, // 题注容器
        indexObj = gUndefined,
        anchor,
        dataForSearch;
    ...
    captionPrefix = V_ui_span(_, V_attr(_data_up_symbol_, Caption_up_symbol) +___+ V_attr(_data_down_symbol_, Caption_down_symbol),
        captionPrefix + Caption_splitter);

    // 尝试获得带题注语法的内容
    let fcSet = Caption_getCaptions(target, type, type);
    ...
    // 指定题注时生成 id 属性用于交叉引用
        ...
        containter.p().a(_id_, LinkTool_transSpecHash(fc));
        containter.p().a(_data_cap_for_id_, fc);
    }
    ...
    // 无题注内容时，整个题注项内容清空
    caption = (V_isLength0(fc.x()) ? _ : captionPrefix + fc);

    // ----------------------------------------
    let cap1Res = Caption_genCap1(fc, captionPrefix),
        cap1 = cap1Res.cap1;
    caption = cap1Res.caption;
    if (cap1Res.fc !== fc)
        fc = cap1Res.fc;

    // ----------------------------------------
    let has2Captions = (fc2 != gNull && V_length(fc2) > 0); // 计算题注数量
    // 有两个题注时
    if (has2Captions) {
        // 第 1 题注
    ...
    // 无题注内容时
        ...
        Caption_noCaption(indexObj, type, captionPrefix, anchor, dataForSearch);
}
...
function Caption_genForMedia(target, type, extType) {
    ...
    if (fc == gNull || V_length(fc.x()) === 0) {
        fc = _;
        fcSet = Caption_getCaptions(target, type, extType);
        if (fcSet[0] !== gNull)
            fc = fcSet[0];
        if (fcSet[1] !== gNull)
            fc2 = fcSet[1];
    ...
    captionPrefix = V_ui_span(_, V_attr(_data_up_symbol_, Caption_up_symbol) +___+ V_attr(_data_down_symbol_, Caption_down_symbol),
        captionPrefix + (V_length(fc) > 0 ? Caption_splitter : _));
    ...
    // 指定题注时生成 id 属性用于交叉引用
        ...
        containter.a(_id_, LinkTool_transSpecHash(fc));
        containter.a(_data_cap_for_id_, fc);
    }
    ...
    if (type.sW(`s`)) {
        ...
        target.wrap(V_ui_divExt(anchor, _v_caption_ +___+ _mermaid_, V_attr(_data_fig_type_, type)));
        dataForSearch += target.f(_div_ + `,` + _text_).t();
    }
    // 为插图（img）增加题注 <img>
    else if (type.sW(`i`)) {
        anchor = _vk_id_fig_ + V_doc_counter_figure;
        target.wrap(V_ui_divExt(anchor, _v_caption_ +___+ _figure_
            ...
            V_attr(_data_fig_type_ , type)));
        dataForSearch += `(${filename}) `;
    }
    // 为音频-非内嵌 tag 增加题注
    else if (type.sW(`a`)) {
    ...
    // 无题注内容，但有 src 值时
    else if (src !== gUndefined && indexObj !== gUndefined) {
        dataForSearch += V_isLength0(dataForSearch) ? `(${filename}) ` : _;
        V_byID(anchor).a(_data_title_, caption);
        indexObj.add(V_ui_span(_, _, captionPrefix + Caption_splitter) + `(${filename}) `, anchor, dataForSearch);
    }
    // 其他情况
    else
        Caption_noCaption(indexObj, type, captionPrefix, anchor, dataForSearch);
 ...
 * 生成第 1 题注内容
 * @param fc 解析后的题注内容
 * @param captionPrefix 题注前缀
 * @returns
 */
function Caption_genCap1(fc, captionPrefix) {
    // ----------------------------------------
    let cap1, caption;
    // 指定只显示题注分类前缀
    if (Caption_mini === _on_) {
...
function Caption_noCaption(indexObj, typeName, captionPrefix, anchor, dataForSearch) {
    ...
    V_byID(anchor).a(_data_title_, caption);
    indexObj.add(V_ui_span(_, _, captionPrefix + Caption_splitter)
        + V_lang_text(99, [
            `( 无题注 )`,
            `( No ${_Caption_} )`
        ]),
 ...
 * @param extType 扩展信息
 */
function Caption_getCaptions(target, type, extType = _) {
    ...
    // 若成功匹配出题注，且内容不是 <img> 则移除原始内容
    if (hideCaptionSrc && (!type.sW(`i`) || capResult[0] === 200))
        caption.rm();
    ...
    /**
     * 返回题注数据数组
     * @param capObj 题注数据的目标对象
     * @param count 题注数量
     * @param type 题注语法类型，100：标准，200：简化语法
     * @returns [] [0] 为第 1 个题注，[1] 为第 2 个题注
     */
    function __getCaptionSet(capObj, count, type) {
        let tempFcSet = [];
        // 标准语法
        if (type === 100) {
            let text = V_ui_escapeHTML(capObj.t().x());
            tempFcSet[0] = text.ss(2, text.i(`]`, 2));
            if (count === 2)
                tempFcSet[1] = text.ss(text.i(`]"`, 2) + 2, V_length(text) - 1);
        }
        // 简化语法
        else if (type === 200) {
            if (count === 2)
                tempFcSet[1] = capObj.f(_mark_ + `>em`).t().x();
            capObj.f(_mark_ + `>em`).rm();
            tempFcSet[0] = V_ui_escapeHTML(capObj.t().x());
        }
        return tempFcSet;
    }
}
...
/**
 * 解析并返回匹配的题注数量
 * @param capTarget 题注语法内容对象
 * @returns [] [0] 语法类型, [1] 题注数量
 */
function Caption_parseSyntax(capTarget) {
    let reg2 = /^!\[.+]".+"$/, // 有 2 个题注
        reg1 = /^!\[.+]$/, // 只有 1 个题注
        html = _;
    // 标准语法，双题注
    if (reg2.test(capTarget.t().x()))
        return [100, 2]; // 100: 标准语法
    // 标准语法，单题注
    else if (reg1.test(capTarget.t().x()))
        return [100, 1]; // 100: 标准语法
    // 简化的新语法
    else if ((html = capTarget.hm()) !== gUndefined) {
...
function Print_preprocessResource() {
    ...
    for (let i = 0; i < Print_imageCount; i++) {
        ...
        // 未加载完成
            ...
            // 绑定加载失败事件
            img.onerror = err => {
                WARN(_image_ + ` ${img.src} ${_load_} `+ _error_, err);
                Print_imageFailedCount++;
                Print_checkProcessing();
            };
        }
    ...
    function __addTab(tabBox, tabGroupWrapper, target, type, current = gFalse) {
        ...
        // 针对表格、插图、代码块、数学公式
            ...
            // 题注分类与编号不显示
                ...
                tabTitle = V_textNodeContent(cap1).x();
                if (V_isLength0(tabTitle))
                    tabTitle = tabTitleAlt;
                // 当无题注内容时，显示题注分类与编号
                if (V_isLength0(tabTitle))
        ...
        // 添加 页签 控件
        let tabItem = $(V_ui_span(
            (current ? _current_ : _) + alertType,
            V_attr(_data_tab_for_, id)
                + V_attr(_tabindex_, 1)
                + V_attr(_data_content_type_, type), icon + tabTitle));
        tabGroupWrapper.ap(tabItem);
        // 绑定标签项的事件处理
        tabItem.ck(() => {
            TabGroup_disposeTabItemClick(tabItem);
            V_ui_setScrollIntoView(tabItem, _center_, _nearest_);
        });
        tabItem.on(_focus_, () => {
            tabItem.tr(_click_);
        });
 ...
 * @param item 页签组条目对象
 */
function TabGroup_tabItemClick(item) {
    ...
    V_ui_setScrollIntoView(scrollTarget, gUndefined, _nearest_);
}
...
function ExtCodeBlock_init() {
    ...
    $(gSelector_codeblock + V_not(V_attrCSS(_data_tab_group_))).e((index, element) => {
        ...
        // 绑定内容助手
        ContentHelper_bind(codeblock, _codeblock_);

        // 生成代码块插图题注（行数 > 1 的才进行处理）
        V_doc_counter_codeblock++;
        // 添加/标准化题注容器
        Caption_wrapContainer(codeblock, _codeblock_);

        // 先根据题注语法生成题注
        Caption_genForText(codeblock, _codeblock_);
    });
}
...
function ExtCodeBlock_copyContent(source) {
    let content = _,
        lines = ContentHelper_lastHover.ch().f(`.${_CodeMirror_code_} .` + _CodeMirror_line_),
        lineNum = 0,
...
function CodeblockWrap_toggle() {
    let codeblock = ContentHelper_lastHover;
    // 已开启，则关闭
...
function CodeblockWrap_isWrap() {
    return JQ_hasClass(ContentHelper_lastHover, _unwrap_);
}
...
function ExtMath_init() {
    ...
    $(_md_math_container_parents_).e((index, element) => {
        ...
        // 绑定内容助手
        ContentHelper_bind(_t, _formula_);
...
function Copy_init() {
    ...
    jqDocument.on(_copy_, event => {
        ...
        let range = selection.getRangeAt(0),
            ...
            finalText = _,
            clipData = V_eventOriginalEvent(event).clipboardData,
            // 常见块级标签集合（可自行扩展）
            blockTags = new Set([
        ...
        function __traverse(node) {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                return JS_cloneNode(node, gTrue); // HTML 模式直接复制
            }

            if (node.nodeType === Node.ELEMENT_NODE) {
                let clNode = JS_cloneNode(node, gFalse), // 克隆当前元素（不含子节点）
                    children = [];
        ...
        // 对 HTML 内容写入剪贴板
        clipData.setData(_text_ + `/` + _html_, JS_innerHTML(htmlFragment));
...
function Unpublish_byYaml() {
    ...
    // --------------------
    // 遍历所有分级章节
    $(_idWrite_ + `>:is(${_h1__6_})`).e((index, element) => {
        let _t = $(element),
            text = _t.t(),
...
function Unpublish_byStrike() {
    // --------------------
    // 指定的引用块（小标题、折叠）
    // 注意：在 _del_ 后增加 __only_child_ 会导致在直接导出 PDF 时无法匹配
    // 但这会导致仅部分内容有删除线时也按不发布处理
    ...
    for (let i = 1; i <= 6; i++) {
        // 注意：在 _del_ 后增加 __only_child_ 会导致在直接导出 PDF 时无法匹配
        // 但这会导致仅部分内容有删除线时也按不发布处理
        $(_idWrite_ + `>h${i}>` + _del_).e((index, element) => {
            // 移除指定的章节及子章节的内容
    ...
    // 针对直接导出 PDF 时的目录
    if (gPrint_typora)
        $(_idWrite_ + ` .` + _md_toc_item_ + `:has(` + _del_ + `)`).rm();
}
...
function Columns_hideHr(hr, remove = gFalse) {
    ...
    if (colCount > 2)
        remove
            ? prevSet.rm()
            : JQ_addClass(prevSet, _v_column_);

    // 处理最后一个 hr
    remove
        ? hr.rm()
        : JQ_addClass(hr, _v_column_);
...
function ExtTable_init() {
    ...
    $(_table_).e((index, element) => {
        ...
        V_doc_counter_table++;
        table.a(_data_num_, V_doc_counter_table);

        // 绑定内容助手
        ContentHelper_bind(table, _table_);

        // 表格冻结列模式为 auto, on 时的处理
        if ((_auto_ + _on_).i(freezeCol) > -1) {
            freezeCol === _on_
                && JQ_addClass(table, _v_freeze_);
        ...
        // 添加/标准化题注容器
        container = Caption_wrapContainer(table, _table_);
        ...
        // 先根据题注语法生成题注
        Caption_genForText(table, _table_);
    }); // table
    sw.ed(`    ├ Prepare: `);
    ...
    $(_table_ + ` .` + _v_empty_cell_).e((index, element) => {
        ...
        if (JQ_hasClass(cell, _v_empty_cell_)) {
            ...
            // 纵向单元格的处理
            tbl.f(V_attrCSS(_data_tbl_col_, cell.a(_data_tbl_col_))).t().x() === _
...
function TableCross_toggle(event) {
    let table = ContentHelper_lastHover;
    // 已打开，则关闭
...
function TableWrap_isWrap() {
    return ContentHelper_lastHover.c(_white_space_) === _pre_wrap_;
}
            ...
            tbodyCells.e((i, e) => {
                ...
                // 内容为百分数
                    ...
                    let coloring = ColumnFormatting_coloringNumber(ce, text, gTrue),
                        ...
                        bg1 = V_ui_var(___ac__ + `wt` + __fd_), // V_ui_formatRGBA(percentBg, 0.1),//`rgba(128, 128, 128, 0.1)`,
                        bg2 = V_ui_var(___ac__ + `t1` + __fd_), // V_ui_formatRGBA(percentBg, 0.4),//`rgba(128, 128, 128, 0.4)`,
                        bgSplit = V_ui_var(___ac__ + `t1`); // V_ui_formatRGBA(percentBg, 0.8);//`rgba(128, 128, 128, 0.8)`;

                    // 对有着色的进行渐变颜色调整
                    if (coloring) {
                        let baseColor = JQ_hasClass(ce, _v_tbl_col_fmt_num_negative_) ? `gn` : `rd`;
                        bg1 = V_ui_var(___ac__ + `wt` + __fd_);
                        bg2 = V_ui_var(___ac__ + baseColor + __fd_);
                        bgSplit = V_ui_var(___ac__ + baseColor);
                    }
                    // 根据百分比生成渐变背景色、最小宽度
...
let RowGroup_folderCount = 0, // 折叠行内行分组类型的数量
    ...
    RowGroup_splitter = `> `, // 行折叠语法与内容的分隔标识
...
function RowGroup_init(table) {
    ...
    table.f(`td` + __first_child_).e((index, element) => {
        ...
        // 从语法中获得当前缩进等级
        currentLevel = text.i(RowGroup_splitter) + 1;
        // 当前等级比上一次等级要深
...
function RowGroup_newFolder(tr) {
    ...
    let td = folderRow.ch(`td:` + _first_),
        tdHadIdent = td.f(`.` + _v_tbl_row_g_ident_ + __last_);
    ...
    JQ_addClass(tdHadIdent, _v_tbl_row_g_ident_folder_);

    // 添加代表目录的括号及样式
    // 重新组合生成新的单元格内容，以支持原始带格式的单元格内容
    let preClass = `.${_v_tbl_row_g_ident_},` + __v_tbl_row_g_btn_,
...
function RowGroup_ident(tr, td, level) {
    ...
    for (let i = 0; i < level; i++) {
        let ident = td.f(`.` + _v_tbl_row_g_ident_ + __last_),
            identObj = V_ui_label(_v_tbl_row_g_ident_, _);
        V_length(ident) > 0
            ? ident.af(identObj)
            : td.pp(identObj);
    }
...
function ExtAudio_init() {
    // 先处理内嵌 <audio> 标签的情况
    $(_audio_).e((index, element) => {
        let audio = $(element);
        V_doc_counter_audio++;
        // 添加/标准化题注容器并返回
        Caption_wrapContainer(audio, _audio_);
        // 生成题注
        Caption_genForMedia(audio, _audio_);
    });
    ...
    $(writeImg + V_attrCSS(_src_, _mp3_, `$`) + `,` + writeImg + V_attrCSS(_src_, _m4a_, `$`) + `,` + writeImg + V_attrCSS(_src_, _ogg_, `$`) + `,` + writeImg + V_attrCSS(_src_, _wav_, `$`) + `,`
        ...
        if (params[_controls_] === _mini_) {
            ...
            // 添加音频播放 mini 控件
            audio.af(V_ui_divExt(_vk_id_mini_audio_ + V_doc_counter_audiomini + __control_, _v_audio_mini_control_ +___+ _disabled_,
                V_attr(_data_title_, _mini_ +___+ _audio_ +___+ V_doc_counter_audiomini), _));

            let controls = V_byID(audio.a(_id_) + __control_);
            // 绑定点击事件
            controls.ck(event => {
                if (JQ_hasClass(controls, _disabled_)){
                    V_preventDefault(event);
                    return;
                }
            ...
            audio.on(_playing_, event => {
                let pause = controls.a(_data_pause_);
        ...
        // 标准控件模式
            ...
            // 添加/标准化题注容器
            Caption_wrapContainer(audioLike, _audio_);
...
function ExtVideo_init() {
    // 先处理内嵌 <audio> 标签的情况
    $(_video_).e((index, element) => {
        let video = $(element);
        V_doc_counter_video++;
        // 添加/标准化题注容器并返回
        Caption_wrapContainer(video, _video_);
        // 生成题注
        Caption_genForMedia(video, _video_);
    });
 ...
 */
function SearchBar(target, id, append) {
    let T = this;
    V_length(T, 1);
    // 文本输入框属性
    T.ui = gUndefined;
    ...
    T.__appendTo = (target, id, append) => {
        // 文本输入框 UI
        let ui = V_ui_div(_, _v_search_ +___+ id,
                    V_ui_div(_, _v_search_ + __icon_)
                    + V_ui_input(_, _, _text_)
                    + V_ui_div(_, _v_search_ + __action_)
                    + V_ui_div(_, _v_search_opt_ +___+ _reset_ +___+ _disabled_, V_ui_svgIcon(_icoResetInput_, 16, 16, _alpha_))
                    + V_ui_div(_, _v_search_opt_ +___+ _highlight_, V_ui_svgIcon(_icoSearchHighlight_, 16, 16, _theme_))
                    + V_ui_div(_, _v_search_opt_ +___+ _doc_lib_, V_ui_svgIcon(_icoSearchDocLib_, 16, 16, _theme_))
                );
        if (append) {
            target.ap(ui);
            T.ui = target.ch(`.${_v_search_}.` + id);
        }
        else {
            target.af(ui);
            T.ui = target.p().ch(`.${_v_search_}.` + id);
        }

        // 获得实例的各关键对象
        T.in = T.ui.ch(_input_);
        T.hl = T.ui.ch(`.` + _v_search_opt_ + `.` + _highlight_);
        T.dl = T.ui.ch(`.` + _v_search_opt_ + `.` + _doc_lib_);
        T.rst = T.ui.ch(`.` + _v_search_opt_ + `.` + _reset_);
        ...
        T.in.on(_focus_, () => {
            ...
            JQ_addClass(T.ui, _v_search_focus_);
            __checkValueChanged();
        ...
        T.in.on(_blur_, () => {
            ...
            JQ_removeClass(T.ui, _v_search_focus_);
        ...
        T.hl.ck(() => {
            // 模拟输入关键字方式进行目录大纲内容过滤
            NavCenter_filterText().l() === _highlight_
                ? NavCenter_clearFilter()
                : Index_filterTocByInput(_highlight_);
        });

        // 鼠标悬停时，显示「高亮」按钮的筛选提示
        T.hl.on(_mouseenter_, () => {
            T.in.a(_data__ + _placeholder_, T.in.a(_placeholder_));
            T.tips(V_lang_text104() + ` → ` + V_lang_text105());
        }).on(_mouseleave_, event => {
            T.tips(T.in.a(_data__ + _placeholder_));
        });
        ...
        T.dl.ck(() => {
            // 模拟输入关键字方式进行目录大纲内容过滤
            NavCenter_filterText().l() === _doclib_
                ? NavCenter_clearFilter()
                : Index_filterTocByInput(_doclib_);
        });

        // 鼠标悬停时，显示「文库」按钮的筛选提示
        T.dl.on(_mouseenter_, () => {
            T.in.a(_data__ + _placeholder_, T.in.a(_placeholder_));
            T.tips(V_lang_text104() + ` → ` + V_lang_text7());
        }).on(_mouseleave_, event => {
            T.tips(T.in.a(_data__ + _placeholder_));
        });
     ...
     */
    T.setIcon = icon => {
        T.icon = T.ui.ch(`.` + _v_search_ + __icon_);
        T.icon.hm(icon);
     ...
     */
    T.setAction = icon => {
        T.act = T.ui.ch(`.` + _v_search_ + __action_);
        T.act.hm(icon);
        T.act.show();
...
function TocIndex(holder, hidden) {
    ...
    T.nm = _toc_;
    T.ui = {
        title : gUndefined, // 有文库项目时指向当前文档的条目
    ...
    T.add = item => {
        ...
        V_length(a.ch(_mark_)) > 0
            && item.a(_data_tips_, _highlight_);
        ...
        if (T.lastFd !== gUndefined) {
            ...
            if (lv > T.lastLv) {
                ...
                folder.ck(event => {
                    ...
                    T.toggleFolder(id, (V_byID(id).a(_data_folded_).sW(`t`)) ? _open_ : _close_, gTrue);
                });
            }
    ...
    T.focusHeader = scrollTop => {
        ...
        function __expandUpFolder(id) {
            if (id === gUndefined)
                return;
            // 展开
            T.toggleFolder(id, _open_, gTrue);
     ...
     */
    T.show = () => {
        if (V_length(NavCenter_filterText()) > 0) {
            T.ui.body.hide();
            T.showRes();
        }
        else {
     ...
     * 处理展开或收起指定章节下的子章节
     * @param id 对象的id值
     * @param action 执行动作（open, close)
     * @param traversal 是否遍历子元素
     */
    T.toggleFolder = (id, action, traversal) => {
        let lastItem = gNull,
            item = V_byID(id),
        ...
        for (let i = 0, len = V_length(itemSet); i < len; i++) {
            ...
            if (traversal
                ...
                && action === _close_) {
                    item.a(_data_folded_, _true_);
                    // 递归处理
                    T.toggleFolder(item.a(_id_), action, traversal);
            }

            // 收起 / 展开
            item.c(_display_, (action === _close_) ? _none_ : _flex_);
            lastItem = item;
        }
        ...
        let indexOfTocH,
            ...
            tocH = _md_toc__ + `h`,
            _t = T.ui.title;
        if (_t !== gUndefined && (indexOfTocH = item.a(_class_).i(tocH)) > -1) {
            indexOfTocH = indexOfTocH + V_length(tocH);
            level = item.a(_class_).ss(indexOfTocH, indexOfTocH + 1);
            if (level === `1`)
                if (V_length(gVlookToc.ch(`.${_md_toc__}h` + level + V_attrCSS(_data_node_, 1) + V_attrCSS(_data_folded_, _false_))) > 0) {
                    JQ_addClass(_t, _open_);
                    _t.a(_data_title_, `❯❯ ` + V_lang_text(102, [
                        `全部折叠`,
                        `Collapse all`
                    ]));
                }
                else {
                    JQ_removeClass(_t, _open_);
                    _t.a(_data_title_, V_lang_text35());
                }
        }
    }
}
...
function Index_noneKeyword(indexObj) {
    ...
    JQ_removeAttr(items, _data_keyword_match_);
 ...
 * @param keyword 指定关键字
 */
function Index_filterTocByInput(keyword) {
    // 选中目录分段
    iNavCenter.sc.setChecked(_toc_);
    // 设置关键字搜索的内容
    iNavCenter.kw.processInput(keyword);
}
...
function Index_scrollToCurrent(indexObj) {
    ...
    if (itemPosTop <= offset)
        V_ui_setScrollTop(itemPosTop + scrollTop - offset, gUndefined, container);
    // 若当前节点在可视区域的下方，则滚动到该节点的位置
    else if (V_ui_getRect(indexObj.curr[0]).bottom > V_ui_getRect(container[0]).bottom - offset)
        V_ui_setScrollTop(itemPosTop + scrollTop - container.h() + offset * 1.5, gUndefined, container);
}
...
function NavHistory() {
    ...
    T.add = hash => {
        ...
        if (V_isLength0(result)) {
            ...
            // 为新增加 / 移动后的记录添加鼠标事件
            newItem.ck(() => {
                V_ui_tg_currentItem(T.ui, newItem);
...
function DocLib(mask, holder) {
    ...
    T.init = () => {
        ...
        if (docLibValue !== gUndefined) {
            let docLibList = V_parseMarkdown_linkSet(docLibValue),
                item = _,
                page = _,
                hash = _,
                text = _,
            ...
            for (let i = 0; i < V_length(docLibList); i++) {
                item = docLibList[i].x();
                result = V_parseMarkdown_link(item);
        ...
        // 给目录大纲添加对应当前文档的标题
        if (V_length(T.fstToc) > 0) {
            T.fstToc.before(V_ui_divExt(_, _v_toc_title_ +___+ _current_, V_attr(_data_title_, V_lang_text35()), T.title));
            T.toc.ch(`.${_md_toc_item_}:` + _last_).af(V_ui_div(_, _v_toc_title_, _));

            T.hdr.toc.ui.title = V_byClass(_v_toc_title_ + `.` + _current_);
            let _t = T.hdr.toc.ui.title;
            _t.ck(() => {
                // 全部折叠
                V_collapseToc(1);
                _t.a(_data_title_, V_lang_text35());
        ...
        function __addDocLibItem(index, text, page, hash, title) {
            ...
            let target = V_getUrlQueryParams(page)[_target_],
                ...
                extLink = (page.i(`://`) > -1),
                fillStyle = (gNavCenterStyle === _edge_ ? _btnFc_ +___ : _) + _current_color_,
                innerIcon = (isSelf
            ...
            // 设置用于被关键字搜索的内容
            item.a(_data_keywords_, _doclib_ + (title === gUndefined ? _ : title));
            item.ck(() => {
                if (hasTarget) {
                    // 去掉 target 参数内容
    ...
    T.addHashItem = (icon, text, hash) => {
        ...
        item.a(_data_keywords_, _doclib_);
...
function ExtFigure_init() {
    ...
    + _idWrite_ + ` .${_md_diagram_panel_}>` + _svg_).e((index, element) => {
        ...
        if (src !== gUndefined) {
            ...
            if (!isPostcard)
                // 添加/标准化题注容器
                container = Caption_wrapContainer(fig, tagName);
        }
        // 对 mermaid 类插图的处理
        else
            // 添加/标准化题注容器
        ...
        if (!isPostcard) {
            // 绑定内容助手
            src !== gUndefined
                ? ContentHelper_bind(fig, _fig_ + _suffixImg_)
                : ContentHelper_bind(fig, _fig_ + _suffixSvg_);
            ...
            JQ_removeClass(container, _v_cap_cntr_);
        }
    }); // 结束初始化处理
    sw.ed(`    ├ figure set: `);
    ...
    function __initPostcard(img, src, hash) {
        ...
        let anchor = _vk_id__ + `psc` + V_doc_counter_postcard,
            caption = V_ui_span(_, _, Caption_prefix(img, V_lang_text68(),  V_doc_counter_postcard) + Caption_splitter) + titleText,
...
function ExtFigure_copySrc(source) {
    Copy_action(source, ContentHelper_lastHover.a(_src_));
 }
    ...
    $(_img_ + V_attrCSS(_data_img_fill_, _text_)
        ...
        // 其他情况
            ...
            if (fill === _text_)
                ...
                fig.c(_filter_, dropShadowPrefix + V_ui_var(___ac__ + fill + __lg_) + `)`);
...
/**
 * 返回当前视图名称标识，如 normal、slide、zen 等
 */
function View_current() {
    if (gLastViewMode === gUndefined)
        return _normal_;
    return gLastViewMode.nm;
}

// ==================== 幻灯片模式 ==================== //

let gSlideObjectSet = gUndefined,
    gSlideGroupIndex = 0,
    gSlideGroupCount = 0,
    gSlideStartScollTop = 0;
...
function SlideView() {
    ...
    T.enable = () => {
        ...
        gSlideStartScollTop = V_ui_getScrollTop();

        gSlideObjectSet = $(_idWrite_ + ` > *`);
        // 首次激活时进行初始化
        if (!T.init) {
            T.init = gTrue;

            gSlideObjectSet.e((index, element) => {
                let _t = $(element);
                // 从 h1-h5 开始分组编号
                if (_h1__5_.i(V_tagName(_t)) > -1)
                    gSlideGroupCount++;
                else {
                    let cls = _t.a(_class_);
                    if (cls !== gUndefined && cls.i(_footnotes_area_) > -1)
                        gSlideGroupCount++;
                }

                // 未开始分组编号前的内容都忽略
                gSlideGroupCount > 0
                    && _t.a(_data_slide_group_, gSlideGroupCount);
            });

            V_ui_hasCover()
                ? VOM_cover().a(_data_slide_group_, 0)
                : VOM_docTitle().a(_data_slide_group_, 0);
            gSlideGroupCount++;
        }
        else
            gSlideGroupIndex = 0;

        iInfoTips.bubble(V_lang_text4() +___+ V_ui_strong(V_lang_text103()) + _2br_
            + V_ui_sup(_, _, V_ui_wrap_kbd(`←`) +___+ V_lang_text15()  + _4space_ + `/` + _4space_)
            + V_ui_sup(_, _, V_lang_text16() +___+ V_ui_wrap_kbd(`→`)) + _2br_
            + V_ui_sup(_, _, V_ui_wrap_kbd(_ESC_) +___+ V_lang_text17())
            , 3000, gUndefined, `og`);
        ...
        // 先隐藏所有内容
        gSlideObjectSet.e((index, element) => {
            JQ_addClass($(element), _v_slide_hide_);
        });
    ...
    T.disable = () => {
        // 先隐藏所有内容
        gSlideObjectSet.e((index, element) => {
            JQ_removeClass($(element), _v_slide_hide_);
        });
        ...
        V_ui_setScrollTop(gSlideStartScollTop);
    ...
    T.next = () => {
        T.hideG();
        gSlideGroupIndex < gSlideGroupCount - 1
            && gSlideGroupIndex++;
    ...
    T.showG = () => {
        // 在状态栏显示页码
        StsFocusInfo_ui.hm(
            V_ui_label(_, _, iToolbar.btns[T.nm].hm() + T.desc + _2nbsp_ + `• ` + (gSlideGroupIndex + 1) + ` / ` + gSlideGroupCount + ` •` + _2nbsp_  + V_ui_wrap_kbd(_ESC_) +___+ V_lang_text17())
        );

        $(_idWrite_ + ` > [${_data_slide_group_}=${gSlideGroupIndex}]`).e((index, element) => {
            JQ_removeClass($(element), _v_slide_hide_);
        });
...
function Restyler_forMermaid() {
    ...
    let seqSelector = `.${_md_diagram_panel_} ` + _svg_ + V_attrCSS(_ariaRoledescription_, `sequence`),
        keySys = /^\*\*.+/g, // 重要系统角色
        extSys = /^--.+/g, // 外部系统角色
        _text_loopText_ = `text.loopText`,
        _text__loopText_tspan = _text_loopText_ + `>tspan`,
        __labelBox_ = `.labelBox`;
    ...
    $(seqSelector +___+ _polygon_ + ` +.labelText`).e((index, element) => {
        ...
        if (segType === _alt_) {
            bgColor = _var_ac_vn_;
            borderColor = _var_ac_vn_;
            titleColor = _var_ac_vn_lg_;
        }
        // 为 loop 片断设置样式
        ...
        // 为 break 片断设置样式
        else if (segType === _break_) {
            bgColor = _var_ac_bk_;
            borderColor = _var_ac_bk_;
            titleColor = _var_ac_bk_lg_;
        }
        // 为 critical 片断设置样式
        else if (segType === _critical_) {
            bgColor = _var_ac_rd_;
        ...
        if (segType !== `opt`) {
            // 背景色
            g.f(_polygon_ + __labelBox_).c(_cssText_, _fill_ + `:` + bgColor + _css_important_);
            // 边框色
            g.f(`line.loopLine`).c(_cssText_, _stroke_ + `:` + borderColor + _css_important_);
            // 片断标题颜色
            g.f(_text_loopText_ + `,` + _text__loopText_tspan).c(_cssText_, _fill_ + `:` + titleColor + _css_important_);
        }
        ...
        if (segType === _alt_)
            ...
            fragment.t(V_lang_text(100, [
                `中断`,
                `Break`
            ]));
        else if (segType === _critical_)
            fragment.t(V_lang_text(101, [
                `关键`,
                `Critical`
            ]));
    });
    ...
    // 调整片断的标题文本
    $(seqSelector +___+ _text__loopText_tspan).e((index, element) => {
        // 去掉文本内容前后的中括号
        let fragmentTitle = $(element);
        fragmentTitle.p().a(_style_, `text-anchor:`  +_start_);

        // 调整文本的位置
        let label = fragmentTitle.p().p().f(__labelBox_),
...
function TextPhonetic_buildForSup(target) {
    ...
    // 绑定注音的点击事件
    targetPrev.f(`${_ruby_}>rt`).ck(event => {
...
function RainbowTextAndCell_build(colorCode, colorSet) {
    ...
    function __disposeText(obj) {
        ...
        if (solid) {
            ...
            obj.c(_color_, V_ui_var(___ac__ + color + title));
        }
        // 渐变色
        else {
    ...
    function __disposeUnderline(obj) {
        ...
        // 单色
        solid
            ? obj.c(_border_color_, V_ui_var(___ac__ + color))
            // 渐变色
    ...
    function __disposeMark(obj) {
        ...
        // 单色
        solid
            ? obj.c(_box_shadow_, `0 ` + V_ui_var(__mark_bg_ + `-h-inset`) + ` 0 0 ` + V_ui_var(___ac__ + color + __fd_) +___+ _inset_)
            // 渐变色
    ...
    function __disposeCell(obj) {
        ...
        // 单色
        solid
            ? obj.c(_background_color_, V_ui_var(___ac__ + color + __fd_))
            // 渐变色
...
let ColorCode = `(rd|ro|og|ye|la|lm|g[nyd]|aq|au|cf|cy|wt|b[unk]|se|sk|vn|p[luk]|mn|wn|ol|t[12u])`,
    ...
    ColorTooLight = /ye|lm|aq|sk|la|pl|pk|gd|cy/i; // 属于亮度较高，用于文字显示时须要降低亮度的颜色标识
...
function CodeMagic_init() {
    ...
    let stdCount = 0,
        write_em = _idWrite_ + ` em>`,
        notSvg_all = V_not(_svg_ + ` *`);
    ...
    // 处理刮刮卡（新语法）
    $(write_em + _span_ + __first_child_ + `+` + _strong_ + __last_child_ + notSvg_all
        + `,` + write_em + _strong_ + __only_child_ + notSvg_all).e((index, element) => {
        let _t = $(element),
            result = [],
            tip = _t.pr();
...
function Tag_build(target, result) {
    ...
    if (!solid) {
        ...
        target.c(_cssText_, _background_image_ + `:${_linear_gradient_}(${_135deg_},`
            + V_ui_genGradColorCSS(gradientColors, _, __lg_) + `)` + _css_important_
            + (color.i(`wt`) > -1 ? _border_color_ + `:` + _var_db_ + _css_important_ : _)
            + _box_shadow_none_);
    }
    // 针对单色语法
    else
...
function Badge_build(target, result) {
    ...
    if (!solid) {
        ...
        target.p().c(_cssText_, _background_image_ + `:${_linear_gradient_}(${_135deg_},`
            + V_ui_genGradColorCSS(gradientColors, _, __lg_) + `)` + _css_important_
            + (color.i(`wt`) > -1 ? _border_color_ + `:` + _var_db_ + _css_important_ : _)
            + _box_shadow_none_);
    }
...
function Progressbar_build(target, value) {
    ...
    color = V_ui_var(___ac__ + rbText);
    colorAlt = V_ui_var(___ac__ + rbText + `-a` + __lg_);
    ...
    label.c(_background_, `${_linear_gradient_}(${_90deg_}, ${color} ${value}%, ${_var_db_} ${value}%, ${_var_db_} ${_100pc_})`);
...
function VLOOKui_iconSet() {
    ...
    ui += V_ui_symbol(_icoVLOOK_,
        ...
        // 图标集：图标|搜索选项：高亮
        + V_ui_symbol(_icoSearchHighlight_,
            V_ui_path(`M2 9h12a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1v-3a1 1 0 0 1 1-1zm6-7a1 1 0 0 1 1 1v2a1 1 0 1 1-2 0V3a1 1 0 0 1 1-1zm7 1.268a1 1 0 0 1 .366 1.366l-1 1.732a1 1 0 0 1-1.732-1l1-1.732A1 1 0 0 1 15 3.268zm-14 0a1 1 0 0 1 1.366.366l1 1.732a1 1 0 1 1-1.732 1l-1-1.732A1 1 0 0 1 1 3.268z`))
        // 图标集：图标|搜索选项：文库
        + V_ui_symbol(_icoSearchDocLib_,
            V_ui_path(`M12 2a3 3 0 0 1 3 3v6a3 3 0 0 1-3 3H4a3 3 0 0 1-3-3V5a3 3 0 0 1 3-3h8zm.001 4h-8a2 2 0 0 0-2 2h4a2 2 0 1 0 4 0h4l-.005-.15A2 2 0 0 0 12 6zm0-3h-8a2 2 0 0 0-2 2h12a2 2 0 0 0-2-2z`))
        // 图标集：图标|收起遮罩层
        ...
        // 图标集：图标|幻灯片视图 / solid
        + V_ui_symbol(_icoViewSlide_ + _solid_,
            V_ui_path(`M19 0a1 1 0 0 1 0 2h-1v10a1 1 0 0 1-1 1h-6v3h3a1 1 0 0 1 0 2H6a1 1 0 0 1 0-2h3v-3H3a1 1 0 0 1-1-1V2H1a1 1 0 1 1 0-2h18zM8.906 4c-.428 0-.776.331-.776.74v4.52c0 .155.05.305.145.43.249.333.734.41 1.083.172l2.947-2.26a.717.717 0 0 0 0-1.203L9.358 4.137A.803.803 0 0 0 8.906 4z`))
        // 图标集：图标|幻灯片视图 / flat
        + V_ui_symbol(_icoViewSlide_ + _flat_,
            V_ui_path(`M2 2h16v10a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V2z`, _, .5)
            + V_ui_path(`M1 0h18a1 1 0 0 1 0 2H1a1 1 0 1 1 0-2zm8 13h2v3h3a1 1 0 0 1 0 2H6a1 1 0 0 1 0-2h3v-3z`, _, .75)
            + V_ui_path(`M12.305 7.602l-2.947 2.26a.801.801 0 0 1-1.083-.172.716.716 0 0 1-.145-.43V4.74c0-.409.348-.74.776-.74.162 0 .32.048.452.138l2.947 2.26a.717.717 0 0 1 0 1.204z`))
        // 图标集：图标|宁静视图 / solid
        + V_ui_symbol(_icoViewZen_ + _solid_,
            V_ui_path(`M1.411 10c2.659 0 3.867.325 5.008.935a6.36 6.36 0 0 1 2.646 2.646c.61 1.14.935 2.35.935 5.008V20H8.589c-2.659 0-3.867-.325-5.008-.935a6.36 6.36 0 0 1-2.646-2.646C.325 15.279 0 14.069 0 11.411V10h1.411zm3.65 4.379a.5.5 0 1 0-.707.707l3.535 3.535a.5.5 0 1 0 .707-.707zM18 4v.798c0 2.194-.325 3.48-.935 4.621a6.36 6.36 0 0 1-2.646 2.646c-1.14.61-2.427.935-4.621.935H9v-.798c0-2.194.325-3.48.935-4.621a6.36 6.36 0 0 1 2.646-2.646c1.14-.61 2.427-.935 4.621-.935H18zm-4.44 3.732l-2.828 2.829a.5.5 0 0 0 .707.707l2.829-2.829a.5.5 0 0 0-.707-.707zM3.983 0c1.651 0 2.32.186 2.972.534.652.349 1.163.86 1.512 1.512.348.652.534 1.32.534 2.972V6h-.982c-1.651 0-2.32-.186-2.972-.534a3.635 3.635 0 0 1-1.512-1.512C3.186 3.302 3 2.634 3 .982V0h.982z`))
        // 图标集：图标|宁静视图 / flat
        + V_ui_symbol(_icoViewZen_ + _flat_,
            V_ui_path(`M0 10h1.411c2.659 0 3.867.325 5.008.935a6.36 6.36 0 0 1 2.646 2.646c.61 1.14.935 2.35.935 5.008V20H8.589c-2.659 0-3.867-.325-5.008-.935a6.36 6.36 0 0 1-2.646-2.646C.325 15.279 0 14.069 0 11.411V10z`, _, .5)
            + V_ui_path(`M18 4h-.798c-2.194 0-3.48.325-4.621.935a6.36 6.36 0 0 0-2.646 2.646C9.325 8.721 9 10.008 9 12.202V13h.798c2.194 0 3.48-.325 4.621-.935a6.36 6.36 0 0 0 2.646-2.646c.61-1.14.935-2.427.935-4.621V4z`, _, .75)
            + V_ui_path(`M3 0h.982c1.651 0 2.32.186 2.972.534.652.349 1.163.86 1.512 1.512.348.652.534 1.32.534 2.972V6h-.982c-1.651 0-2.32-.186-2.972-.534a3.635 3.635 0 0 1-1.512-1.512C3.186 3.302 3 2.634 3 .982V0zm2.06 14.379l3.536 3.535a.5.5 0 0 1-.707.707l-3.535-3.535a.5.5 0 1 1 .707-.707zm8.5-6.647l-2.828 2.829a.5.5 0 0 0 .707.707l2.829-2.829a.5.5 0 0 0-.707-.707z`))
        // 图标集：图标|聚光灯 / solid
...
function VLOOKui_toolbar() {
    ...
    return V_ui_nav(_, _v_toolbar_ +___+ gNavCenterStyle +___+ _v_focus_out_, _,
            ...
            // 视图切换
            + V_ui_div(_, _v_btn_group_ +___+ _view_,
                // 幻灯片模式
                V_ui_divExt(_, _v_btn_ +___+ _view_ +___+ _slide_, V_attr(_data_btn_group_, _view_),
                    V_ui_svgIcon(_icoViewSlide_ + gIconStyle_uico, 20, 18, _current_color_))
                    // 宁静视图
                + V_ui_divExt(_, _v_btn_ +___+ _view_ +___+ _zen_, V_attr(_data_btn_group_, _view_),
                    V_ui_svgIcon(_icoViewZen_ + gIconStyle_uico, 18, 20, _current_color_))
                )
        );
}
...
function VLOOKui_docNavHistory() {
    return V_ui_nav(_, _v_shortcut_bar_ +___+ _v_float_card_, _,
...
function VLOOKui_common() {
    ...
    // --------------------------------------------------
    // 复制内容的按钮
    let classBtnAssistor = _v_btn_ +___+ _helper_ +___;
    ui += V_ui_div(_, _v_content_helper_ +___+ _v_float_card_,
    ...
    // --------------------------------------------------
    // 页面坏链检查结果及错误列表
    ui += V_ui_aside(_, _v_link_info_list_, _,
    ...
    ui += V_ui_nav(_, _v_status_bar_ +___+ _v_float_card_ +___+ _v_focus_out_, _,
            ...
            + V_ui_div(_, _v_sts_gray_, V_ui_svgIcon(_icoGrayMode_ + gIconStyle_uico, 20, 20, _text_) + V_lang_text48())
...
function V_lazyLoading() {
    ...
    $(_img_ + `,` + gSelector_iframeVideo).e((index, element) => {
        ...
        if (V_length(ext) > 1 && (_mp3_ + _m4a_ + _ogg_ + _wav_ + _mp4_ + _ogv_ + _webm_).i(ext) < 0) {
            ...
            // 根据图片版式处理
            // 不是插图
            if (V_isNotFigure(V_getUrlHash(src), _t))
                JQ_removeAttr(_t, _src_);
            // 插图
            else if (tagName !== _iframe_) {
                gFigurePlaceholder = V_getVarVal(___thm_fig_ph_);
                _t.a(_src_, gFigurePlaceholder);
            }
...
$(() => { // 等价于 $(document).ready()
    ...
    // 待办 to-do: 关注 jQuery v4.0 后是否有解决这个问题
    (() => {
...
function loadVLOOKplugin() {
    ...
    if (gThmColorScheme === _auto_) {
        ...
        // 若没有通过调校参数指定
            ...
            if (colorScheme === gNull)
                colorScheme = V_data_read(_color__scheme_, gTrue);
            // 通过同域配置指定
    ...
    let footerValue = V_getMetaContent(_vlook_ + __footer_);
    ...
    // ----------------------------------------
    // 兼容性检查
    if (V_pageMode === _max_ && (gThmVer !== gVer || gCompCode !== V_getTemplateThemeCompCode())) {
        ALERT([
            _Warning_tt_cn_ + `\n\n本文档使用的 ${_VLOOK_TM_} 主题与插件版本不匹配，会影响实际使用，请将更新至匹配的版本！！！`
                + `\n\n主题 [ ${gThmName} ]\n`
                + `版本 [ ${gThmVer} ]`,
            _Warning_tt_ + `\n\nThe ${_VLOOK_TM_} ${_theme_} and ${_plugin_} ${_version_} ${_use_}d in this ${_document_} do not ${_match_}, which may affect actual ${_use_}. Please ${_update_} to the matching ${_version_} !!!`
                + `\n\n${_Theme_} [ ${gThmName} ]\n`
                + `${_version_} [ ${gThmVer} ]`
        ][V_lang]);

        JQ_removeClass(VOM_write(), _done_);
        return;
    }
    ...
    // ----------------------------------------
    // 移动端上的 Safari / WebKit 的渲染机制会导致动画初始化时无法渲染
    // 需要进行强制触发动画的处理
    if (!gPrintMode && V_ui_hasCover() && V_isMobile()) {
        VOM_cover().rW(VOM_cover().cl());
        VOM_c = gUndefined;
    }
